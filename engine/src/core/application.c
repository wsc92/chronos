...
b8 event_on_debug_event(u16 code, void* sender, void* listener_inst, event_context data) {
    if (code == EVENT_CODE_DEBUG0) {
        ...
        if (g) {
            ...
            if (!g->material) {
                CWARN("event_on_debug_event no material found! Using default material.");
...
b8 application_create(game* game_inst) {
    ...
    if (!job_system_initialize(&app_state->job_system_memory_requirement, app_state->job_system_state, thread_count, job_thread_types)) {
        CFATAL("Failed to initialize job system. Aborting application.");
        return false;
    }
    ...
    if (!texture_system_initialize(&app_state->texture_system_memory_requirement, app_state->texture_system_state, texture_sys_config)) {
        CFATAL("Failed to initialize texture system. Application cannot continue.");
        return false;
    }
    ...
    if (!font_system_initialize(&app_state->font_system_memory_requirement, app_state->font_system_state, &font_sys_config)) {
        CFATAL("Failed to initialize font system. Application cannot continue.");
        return false;
    }
    ...
    if (!camera_system_initialize(&app_state->camera_system_memory_requirement, app_state->camera_system_state, camera_sys_config)) {
        CFATAL("Failed to initialize camera system. Application cannot continue.");
        return false;
    }
    ...
    if (!render_view_system_initialize(&app_state->renderer_view_system_memory_requirement, app_state->renderer_view_system_state, render_view_sys_config)) {
        CFATAL("Failed to initialize render view system. Aborting application.");
        return false;
    }
    ...
    if (!render_view_system_create(&skybox_config)) {
        CFATAL("Failed to create skybox view. Aborting application.");
        return false;
    }
    ...
    if (!render_view_system_create(&world_view_config)) {
        CFATAL("Failed to create world view. Aborting application.");
        return false;
    }
    ...
    if (!render_view_system_create(&ui_view_config)) {
        CFATAL("Failed to create ui view. Aborting application.");
        return false;
    }
    ...
    if (!render_view_system_create(&pick_view_config)) {
        CFATAL("Failed to create pick view. Aborting application.");
        return false;
    }
    ...
    if (!material_system_initialize(&app_state->material_system_memory_requirement, app_state->material_system_state, material_sys_config)) {
        CFATAL("Failed to initialize material system. Application cannot continue.");
        return false;
    }
    ...
    if (!geometry_system_initialize(&app_state->geometry_system_memory_requirement, app_state->geometry_system_state, geometry_sys_config)) {
        CFATAL("Failed to initialize geometry system. Application cannot continue.");
        return false;
    }
    ...
    if (!ui_text_create(UI_TEXT_TYPE_BITMAP, "Ubuntu Mono 21px", 21, "Some test text 123,\n\tyo!", &app_state->test_text)) {
        CERROR("Failed to load basic ui bitmap text.");
        return false;
    }
    ...
    if(!ui_text_create(UI_TEXT_TYPE_SYSTEM, "Noto Sans CJK JP", 31, "Some system text 123, \n\tyo!\n\n\tこんにちは 한", &app_state->test_sys_text)) {
        CERROR("Failed to load basic ui system text.");
        return false;
    }
    ...
    if (!renderer_texture_map_acquire_resources(cube_map)) {
        CFATAL("Unable to acquire resources for cube map texture.");
        return false;
    }
    ...
    if (!renderer_shader_acquire_instance_resources(skybox_shader, maps, &app_state->sb.instance_id)) {
        CFATAL("Unable to acquire shader resources for skybox texture.");
        return false;
    }
    ...
    cube_mesh->geometries = callocate(sizeof(mesh*) * cube_mesh->geometry_count, MEMORY_TAG_ARRAY);
    ...
    cube_mesh_2->geometries = callocate(sizeof(mesh*) * cube_mesh_2->geometry_count, MEMORY_TAG_ARRAY);
    ...
    cube_mesh_3->geometries = callocate(sizeof(mesh*) * cube_mesh_3->geometry_count, MEMORY_TAG_ARRAY);
    ...
    app_state->ui_meshes[0].geometries = callocate(sizeof(geometry*), MEMORY_TAG_ARRAY);
...
b8 application_run() {
    ...
    while (app_state->is_running) {
        ...
        if (!app_state->is_suspended) {
            ...
            czero_memory(views, sizeof(render_view_packet) * packet.view_count);
            packet.views = views;
            ...
            if (!render_view_system_build_packet(render_view_system_get("skybox"), &skybox_data, &packet.views[0])) {
                CERROR("Failed to build packet for view 'skybox'.");
                return false;
            }
