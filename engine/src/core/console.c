...
void console_initialize(u64* memory_requirement, void* memory) {
    *memory_requirement = sizeof(console_state) + (sizeof(console_consumer) * MAX_CONSUMER_COUNT);
    ...
    czero_memory(memory, *memory_requirement);
    state_ptr = memory;
...
void console_shutdown(void* state) {
    if (state_ptr) {
        ...
        czero_memory(state, sizeof(console_state) + (sizeof(console_consumer) * MAX_CONSUMER_COUNT));
    }
...
void console_register_consumer(void* inst, PFN_console_consumer_write callback) {
    if (state_ptr) {
        CASSERT_MSG(state_ptr->consumer_count + 1 < MAX_CONSUMER_COUNT, "Max console consumers reached.");
...
b8 console_register_command(const char* command, u8 arg_count, PFN_console_command func) {
    CASSERT_MSG(state_ptr && command, "console_register_command requires state and valid command");
    ...
    for (u32 i = 0; i < command_count; ++i) {
        if (strings_equali(state_ptr->registered_commands[i].name, command)) {
            CERROR("Command already registered: %s", command);
            return false;
        }
    }
...
b8 console_execute_command(const char* command) {
    ...
    for (u32 i = 0; i < command_count; ++i) {
        ...
        if (strings_equali(cmd->name, parts[0])) {
            ...
            if (state_ptr->registered_commands[i].arg_count != arg_count) {
                CERROR("The console command '%s' requires %u arguments but %u were provided.", cmd->name, cmd->arg_count, arg_count);
                has_error = true;
            } else {
                ...
                if (context.argument_count > 0) {
                    context.arguments = callocate(sizeof(console_command_argument) * cmd->arg_count, MEMORY_TAG_ARRAY);
                ...
                if (context.arguments) {
                    cfree(context.arguments, sizeof(console_command_argument) * cmd->arg_count, MEMORY_TAG_ARRAY);
                }
            }
            break;
        }
    ...
    if (!command_found) {
        CERROR("The command '%s' does not exist.", parts[0]);
        has_error = true;
    }
