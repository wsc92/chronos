...
u32 string_utf8_length(const char* str) {
    ...
    for (u32 i = 0; i < __UINT32_MAX__; ++i, ++length) {
        ...
        if (c >= 0 && c < 127) {
            ...
            CERROR("kstring string_utf8_length() - Not supporting 5 and 6-byte characters; Invalid UTF-8.");
            return 0;
        }
    }
...
b8 bytes_to_codepoint(const char* bytes, u32 offset, i32* out_codepoint, u8* out_advance) {
    ...
    if (codepoint >= 0 && codepoint < 0x7F) {
        ...
        *out_advance = 0;
        *out_codepoint = 0;
        CERROR("kstring bytes_to_codepoint() - Not supporting 5 and 6-byte characters; Invalid UTF-8.");
        return false;
    }
}
...
void string_free(char* str) {
    if (str) {
        u64 size = 0;
        u16 alignment = 0;
        if (cmemory_get_size_alignment(str, &size, &alignment)) {
            cfree_aligned(str, size, alignment, MEMORY_TAG_STRING);
        } else {
            // TODO: report failure?
        }
    } else {
...
b8 string_to_vec4(char* str, vec4* out_vector) {
    ...
    czero_memory(out_vector, sizeof(vec4));
...
b8 string_to_vec3(char* str, vec3* out_vector) {
    ...
    czero_memory(out_vector, sizeof(vec3));
...
b8 string_to_vec2(char* str, vec2* out_vector) {
    ...
    czero_memory(out_vector, sizeof(vec2));
    ...
    for (u32 i = 0; i < length; ++i) {
        ...
        if (c == delimiter) {
            ...
            if (trimmed_length > 0 || include_empty) {
                char* entry = callocate(sizeof(char) * (trimmed_length + 1), MEMORY_TAG_STRING);
                if (trimmed_length == 0) {
                    entry[0] = 0;
                } else {
...
void string_cleanup_split_array(char** str_darray) {
    if (str_darray) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            cfree(str_darray[i], sizeof(char) * (len + 1), MEMORY_TAG_STRING);
        }
