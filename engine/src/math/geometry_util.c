...
b8 vertex3d_equal(vertex_3d vert_0, vertex_3d vert_1) {
    return vec3_compare(vert_0.position, vert_1.position, C_FLOAT_EPSILON) &&
           vec3_compare(vert_0.normal, vert_1.normal, C_FLOAT_EPSILON) &&
           vec2_compare(vert_0.texcoord, vert_1.texcoord, C_FLOAT_EPSILON) &&
           vec4_compare(vert_0.colour, vert_1.colour, C_FLOAT_EPSILON) &&
           vec4_compare(vert_0.tangent, vert_1.tangent, C_FLOAT_EPSILON);
}
...
void geometry_deduplicate_vertices(u32 vertex_count, vertex_3d* vertices, u32 index_count, u32* indices, u32* out_vertex_count, vertex_3d** out_vertices) {
    ...
    vertex_3d* unique_verts = callocate(sizeof(vertex_3d) * vertex_count, MEMORY_TAG_ARRAY);
    *out_vertex_count = 0;
    ...
    *out_vertices = callocate(sizeof(vertex_3d) * (*out_vertex_count), MEMORY_TAG_ARRAY);
    // Copy over unique
    ccopy_memory(*out_vertices, unique_verts, sizeof(vertex_3d) * (*out_vertex_count));
    // Destroy temp array
    cfree(unique_verts, sizeof(vertex_3d) * vertex_count, MEMORY_TAG_ARRAY);

    u32 removed_count = vertex_count - *out_vertex_count;
    CDEBUG("geometry_deduplicate_vertices: removed %d vertices, orig/now %d/%d.", removed_count, vertex_count, *out_vertex_count);
}
