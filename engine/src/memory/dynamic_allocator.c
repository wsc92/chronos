...
#define CSIZE_STORAGE sizeof(u32)

b8 dynamic_allocator_create(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator* out_allocator) {
    if (total_size < 1) {
        CERROR("dynamic_allocator_create cannot have a total_size of 0. Create failed.");
        return false;
    }
    if (!memory_requirement) {
        CERROR("dynamic_allocator_create requires memory_requirement to exist. Create failed.");
        return false;
    }
    ...
    *memory_requirement = freelist_requirement + sizeof(dynamic_allocator_state) + total_size;
    ...
    czero_memory(state->memory_block, total_size);
    return true;
}
...
b8 dynamic_allocator_destroy(dynamic_allocator* allocator) {
    if (allocator) {
        ...
        czero_memory(state->memory_block, state->total_size);
        state->total_size = 0;
        allocator->memory = 0;
        return true;
    }

    CWARN("dynamic_allocator_destroy requires a pointer to an allocator. Destroy failed.");
    return false;
}
...
void* dynamic_allocator_allocate_aligned(dynamic_allocator* allocator, u64 size, u16 alignment) {
    if (allocator && size && alignment) {
        ...
        u64 required_size = alignment + sizeof(alloc_header) + CSIZE_STORAGE + size;
        // NOTE: This cast will really only be an issue on allocations over ~4GiB, so... don't do that.
        CASSERT_MSG(required_size < 4294967295U, "dynamic_allocator_allocate_aligned called with required size > 4 GiB. Don't do that.");
        ...
        if (freelist_allocate_block(&state->list, required_size, &base_offset)) {
            ...
            */
            ...
            u64 aligned_block_offset = get_aligned((u64)ptr + base_offset + CSIZE_STORAGE, alignment);
            // Store the size just before the user data block
            u32* block_size = (u32*)(aligned_block_offset - CSIZE_STORAGE);
            *block_size = (u32)size;
            ...
            CERROR("dynamic_allocator_allocate_aligned no blocks of memory large enough to allocate from.");
            u64 available = freelist_free_space(&state->list);
            CERROR("Requested size: %llu, total space available: %llu", size, available);
            // TODO: Report fragmentation?
            return 0;
        }
    }

    CERROR("dynamic_allocator_allocate_aligned requires a valid allocator, size and alignment.");
    return 0;
}
...
b8 dynamic_allocator_free_aligned(dynamic_allocator* allocator, void* block) {
    if (!allocator || !block) {
        CERROR("dynamic_allocator_free_aligned requires both a valid allocator (0x%p) and a block (0x%p) to be freed.", allocator, block);
        return false;
    }
    ...
    if (block < state->memory_block || block > state->memory_block + state->total_size) {
        ...
        CERROR("dynamic_allocator_free_aligned trying to release block (0x%p) outside of allocator range (0x%p)-(0x%p)", block, state->memory_block, end_of_block);
        return false;
    }

    u32* block_size = (u32*)((u64)block - CSIZE_STORAGE);
    alloc_header* header = (alloc_header*)((u64)block + *block_size);
    u64 required_size = header->alignment + sizeof(alloc_header) + CSIZE_STORAGE + *block_size;
    u64 offset = (u64)header->start - (u64)state->memory_block;
    if (!freelist_free_block(&state->list, required_size, offset)) {
        CERROR("dynamic_allocator_free_aligned failed.");
        return false;
    }
...
b8 dynamic_allocator_get_size_alignment(void* block, u64* out_size, u16* out_alignment) {
    // Get the header.
    *out_size = *(u32*)((u64)block - CSIZE_STORAGE);
