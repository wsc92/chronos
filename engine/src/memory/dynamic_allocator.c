...
void* dynamic_allocator_allocate(dynamic_allocator* allocator, u64 size) {
    if (allocator && size) {
        ...
        if (freelist_allocate_block(&state->list, size, &offset)) {
            ...
            CERROR("dynamic_allocator_allocate no blocks of memory large enough to allocate from.");
    ...
    CERROR("dynamic_allocator_allocate requires a valid allocator and size.");
    return 0;
}

b8 dynamic_allocator_free(dynamic_allocator* allocator, void* block, u64 size) {
    if (!allocator || !block || !size) {
        CERROR("dynamic_allocator_free requires both a valid allocator (0x%p) and a block (0x%p) to be freed.", allocator, block);
        return false;
    }
    dynamic_allocator_state* state = allocator->memory;
    if (block < state->memory_block || block > state->memory_block + state->total_size) {
        ...
        CERROR("dynamic_allocator_free trying to release block (0x%p) outside of allocator range (0x%p)-(0x%p)", block, state->memory_block, end_of_block);
        return false;
    }
    u64 offset = (block - state->memory_block);
    if (!freelist_free_block(&state->list, size, offset)) {
        CERROR("dynamic_allocator_free failed.");
        return false;
    }
    return true;
}
