...
void acquire_shader_instances(const struct render_view* self) {
    ...
    if (!renderer_shader_acquire_instance_resources(data->ui_shader_info.s, 0, &instance)) {
        CFATAL("render_view_pick failed to acquire shader resources.");
        return;
    }
    // World shader
...
void release_shader_instances(const struct render_view* self) {
    ...
    for (u32 i = 0; i < data->instance_count; ++i) {
        ...
        if (!renderer_shader_release_instance_resources(data->ui_shader_info.s, i)) {
            CWARN("Failed to release shader resources.");
        }
...
b8 render_view_pick_on_create(struct render_view* self) {
    if (self) {
        self->internal_data = callocate(sizeof(render_view_pick_internal_data), MEMORY_TAG_RENDERER);
        render_view_pick_internal_data* data = self->internal_data;
        ...
        if (!resource_system_load(ui_shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            CERROR("Failed to load builtin UI Pick shader.");
            return false;
        }
        ...
        if (!resource_system_load(world_shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            CERROR("Failed to load builtin World Pick shader.");
            return false;
        }
        ...
        czero_memory(&data->colour_target_attachment_texture, sizeof(texture));
        czero_memory(&data->depth_target_attachment_texture, sizeof(texture));

        // Register for mouse move event.
        if (!event_register(EVENT_CODE_MOUSE_MOVED, self, on_mouse_moved)) {
            CERROR("Unable to listen for mouse move event, creation failed.");
            return false;
        }

        if (!event_register(EVENT_CODE_DEFAULT_RENDERTARGET_REFRESH_REQUIRED, self, render_view_on_event)) {
            CERROR("Unable to listen for refresh required event, creation failed.");
            return false;
        }

        return true;
    }
    CERROR("render_view_pick_on_create - Requires a valid pointer to a view.");
    return false;
}
...
void render_view_pick_on_destroy(struct render_view* self) {
    if (self && self->internal_data) {
        ...
        cfree(self->internal_data, sizeof(render_view_pick_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_pick_on_build_packet(const struct render_view* self, struct linear_allocator* frame_allocator, void* data, struct render_view_packet* out_packet) {
    if (!self || !data || !out_packet) {
        CWARN("render_view_pick_on_build_packet requires valid pointer to view, packet, and data.");
        return false;
    }
    ...
    ccopy_memory(out_packet->extended_data, packet_data, sizeof(pick_packet_data));
...
b8 render_view_pick_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index) {
    ...
    if (render_target_index == 0) {
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[render_target_index])) {
            CERROR("render_view_ui_on_render pass index %u failed to start.", p);
            return false;
        }
        ...
        for (u32 i = 0; i < world_geometry_count; ++i) {
            ...
            if (!shader_system_uniform_set_by_index(data->world_shader_info.id_colour_location, &id_colour)) {
                CERROR("Failed to apply id colour uniform.");
                return false;
            }
            ...
            if (!shader_system_uniform_set_by_index(data->world_shader_info.model_location, &geo->model)) {
                CERROR("Failed to apply model matrix for world geometry.");
            }
        ...
        if (!renderer_renderpass_end(pass)) {
            CERROR("render_view_ui_on_render pass index %u failed to end.", p);
            return false;
        }
        ...
        for (u32 i = world_geometry_count; i < packet->geometry_count; ++i) {
            ...
            if (!shader_system_uniform_set_by_index(data->ui_shader_info.model_location, &geo->model)) {
                CERROR("Failed to apply model matrix for text");
            }
    ...
    // Clamp to image size
    u16 x_coord = CCLAMP(data->mouse_x, 0, self->width - 1);
    u16 y_coord = CCLAMP(data->mouse_y, 0, self->height - 1);
...
b8 render_view_pick_regenerate_attachment_target(struct render_view* self, u32 pass_index, struct render_target_attachment* attachment) {
    ...
    if (attachment->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
        ...
        CERROR("Unsupported attachment type 0x%x.", attachment->type);
        return false;
    }
    ...
    if (attachment->texture->internal_data) {
        ...
        czero_memory(attachment->texture, sizeof(texture));
    }
