...
b8 render_view_ui_on_create(struct render_view* self) {
    if (self) {
        self->internal_data = callocate(sizeof(render_view_ui_internal_data), MEMORY_TAG_RENDERER);
        ...
        if (!resource_system_load(shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            CERROR("Failed to load builtin UI shader.");
            return false;
        }
    ...
    CERROR("render_view_ui_on_create - Requires a valid pointer to a view.");
    return false;
}
...
void render_view_ui_on_destroy(struct render_view* self) {
    if (self && self->internal_data) {
        ...
        cfree(self->internal_data, sizeof(render_view_ui_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_ui_on_build_packet(const struct render_view* self, struct linear_allocator* frame_allocator, void* data, struct render_view_packet* out_packet) {
    if (!self || !data || !out_packet) {
        CWARN("render_view_ui_on_build_packet requires valid pointer to view, packet, and data.");
        return false;
    }
    ...
    ccopy_memory(out_packet->extended_data, packet_data, sizeof(ui_packet_data));
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p) {
        ...
        for (u32 i = 0; i < packet_data->text_count; ++i) {
            ...
            if (!shader_system_uniform_set_by_index(data->diffuse_map_location, &text->data->atlas)) {
                CERROR("Failed to apply bitmap font diffuse map uniform.");
                return false;
            }
            ...
            if (!shader_system_uniform_set_by_index(data->diffuse_colour_location, &white_colour)) {
                CERROR("Failed to apply bitmap font diffuse colour uniform.");
                return false;
            }
