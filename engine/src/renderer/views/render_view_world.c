...
b8 render_view_world_on_create(struct render_view* self) {
    if (self) {
        self->internal_data = callocate(sizeof(render_view_world_internal_data), MEMORY_TAG_RENDERER);
        ...
        if (!resource_system_load(shader_name, RESOURCE_TYPE_SHADER, 0, &config_resource)) {
            CERROR("Failed to load builtin material shader.");
            return false;
        }
        ...
        if (!event_register(EVENT_CODE_SET_RENDER_MODE, self, render_view_on_event)) {
            CERROR("Unable to listen for render mode set event, creation failed.");
            return false;
        }
    ...
    CERROR("render_view_world_on_create - Requires a valid pointer to a view.");
    return false;
}
...
void render_view_world_on_destroy(struct render_view* self) {
    if (self && self->internal_data) {
        ...
        cfree(self->internal_data, sizeof(render_view_world_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_world_on_build_packet(const struct render_view* self, void* data, struct render_view_packet* out_packet) {
    if (!self || !data || !out_packet) {
        CWARN("render_view_world_on_build_packet requires valid pointer to view, packet, and data.");
        return false;
    }
...
b8 render_view_world_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index) {
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p) {
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[render_target_index])) {
            CERROR("render_view_world_on_render pass index %u failed to start.", p);
            return false;
        }
        ...
        if (!renderer_renderpass_end(pass)) {
            CERROR("render_view_world_on_render pass index %u failed to end.", p);
            return false;
        }
    }
