...
b8 vulkan_ui_shader_create(vulkan_context* context, vulkan_ui_shader* out_shader) {
    ...
    for (u32 i = 0; i < UI_SHADER_STAGE_COUNT; ++i) {
        if (!create_shader_module(context, BUILTIN_SHADER_NAME_UI, stage_type_strs[i], stage_types[i], i, out_shader->stages)) {
            CERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_UI);
            return false;
        }
    }
    ...
    czero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_buffer_create(
        ...
        CERROR("UI instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_ui_shader_apply_material(vulkan_context* context, struct vulkan_ui_shader* shader, material* material) {
    if (context && shader) {
        ...
        czero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
        ...
        for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index) {
            ...
            switch (use) {
                case TEXTURE_USE_MAP_DIFFUSE:
                    ...
                    break;
                default:
                    CFATAL("Unable to bind sampler to unknown use.");
                    return;
            }
...
b8 vulkan_ui_shader_acquire_resources(vulkan_context* context, struct vulkan_ui_shader* shader, material* material) {
    ...
    if (result != VK_SUCCESS) {
        CERROR("Error allocating descriptor sets in shader!");
        return false;
    }
