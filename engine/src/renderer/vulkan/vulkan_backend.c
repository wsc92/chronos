...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    *out_window_render_target_count = context.swapchain.image_count;
    ...
    context.renderpass_table_block = callocate(sizeof(u32) * VULKAN_MAX_REGISTERED_RENDERPASSES, MEMORY_TAG_RENDERER);
    ...
    for (u32 i = 0; i < config->renderpass_count; ++i) {
        ...
        if (id != INVALID_ID) {
            CERROR("Collision with renderpass named '%s'. Initialization failed.", config->pass_configs[i].name);
            return false;
        }
        // Snip up a new id.
        ...
        // Verify we got an id
        if (id == INVALID_ID) {
            CERROR("No space was found for a new renderpass. Increase VULKAN_MAX_REGISTERED_RENDERPASSES. Initialization failed.");
            return false;
        }
...
renderpass* vulkan_renderer_renderpass_get(const char* name) {
    if (!name || name[0] == 0) {
        CERROR("vulkan_renderer_renderpass_get requires a name. Nothing will be returned.");
        return 0;
    }
    ...
    if (id == INVALID_ID) {
        CWARN("There is no registered renderpass named '%s'.", name);
        return 0;
    }
...
b8 vulkan_renderer_shader_create(shader* shader, const shader_config* config, renderpass* pass, u8 stage_count, const char** stage_filenames, shader_stage* stages) {
    shader->internal_data = callocate(sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
    ...
    czero_memory(out_shader->config.stages, sizeof(vulkan_shader_stage_config) * VULKAN_SHADER_MAX_STAGES);
    ...
    czero_memory(out_shader->config.descriptor_sets, sizeof(vulkan_descriptor_set_config) * 2);
    ...
    // Attributes array.
    czero_memory(out_shader->config.attributes, sizeof(VkVertexInputAttributeDescription) * VULKAN_SHADER_MAX_ATTRIBUTES);
...
b8 vulkan_renderer_shader_initialize(shader* shader) {
    ...
    czero_memory(s->stages, sizeof(vulkan_shader_stage) * VULKAN_SHADER_MAX_STAGES);
    for (u32 i = 0; i < s->config.stage_count; ++i) {
        if (!create_module(s, s->config.stages[i], &s->stages[i])) {
            CERROR("Unable to create %s shader module for '%s'. Shader will be destroyed.", s->config.stages[i].file_name, shader->name);
            return false;
        }
    }
    ...
    czero_memory(s->descriptor_set_layouts, s->config.descriptor_set_count);
...
void vulkan_renderpass_create(renderpass* out_renderpass, f32 depth, u32 stencil, b8 has_prev_pass, b8 has_next_pass) {
    ...
    if (do_clear_depth) {
        ...
        czero_memory(&attachment_descriptions[attachment_description_count], sizeof(VkAttachmentDescription));
...
void vulkan_renderer_render_target_create(u8 attachment_count, texture** attachments, renderpass* pass, u32 width, u32 height, render_target* out_target) {
    ...
    if (!out_target->attachments) {
        out_target->attachments = callocate(sizeof(texture*) * attachment_count, MEMORY_TAG_ARRAY);
    }
    ccopy_memory(out_target->attachments, attachments, sizeof(texture*) * attachment_count);
...
void vulkan_renderer_render_target_destroy(render_target* target, b8 free_internal_memory) {
    if (target && target->internal_framebuffer) {
        ...
        if (free_internal_memory) {
            cfree(target->attachments, sizeof(texture*) * target->attachment_count, MEMORY_TAG_ARRAY);
...
texture* vulkan_renderer_window_attachment_get(u8 index) {
    if (index >= context.swapchain.image_count) {
        CFATAL("Attempting to get attachment index out of range: %d. Attachment count: %d", index, context.swapchain.image_count);
        return 0;
    }
