...
void vulkan_image_create(
    ...
    callocate_report(out_image->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
...
void vulkan_image_transition_layout(
    ...
    if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        ...
        CFATAL("unsupported layout transition!");
        return;
    }
...
void vulkan_image_destroy(vulkan_context* context, vulkan_image* image) {
    ...
    cfree_report(image->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
    czero_memory(&image->memory_requirements, sizeof(VkMemoryRequirements));
}
