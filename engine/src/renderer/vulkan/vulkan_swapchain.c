...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        vulkan_swapchain_recreate(context, context->framebuffer_width, context->framebuffer_height, swapchain);
        CDEBUG("Swapchain recreated because swapchain returned out of date or suboptimal.");
...
void create(vulkan_context* context, u32 width, u32 height, renderer_config_flags flags, vulkan_swapchain* swapchain) {
    ...
    if (!swapchain->depth_textures) {
        swapchain->depth_textures = (texture*)callocate(sizeof(texture) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }

    for (u32 i = 0; i < context->swapchain.image_count; ++i) {
        // Create depth image and its view.
        vulkan_image* image = callocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
        vulkan_image_create(
            context,
            TEXTURE_TYPE_2D,
            swapchain_extent.width,
