...
b8 binary_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, true, &f)) {
        CERROR("binary_loader_load - unable to open file for binary reading: '%s'.", full_file_path);
        return false;
    }

    u64 file_size = 0;
    if (!filesystem_size(&f, &file_size)) {
        CERROR("Unable to binary read file: %s.", full_file_path);
        filesystem_close(&f);
        return false;
    }

    // TODO: Should be using an allocator here.
    u8* resource_data = callocate(sizeof(u8) * file_size, MEMORY_TAG_ARRAY);
    u64 read_size = 0;
...
void binary_loader_unload(struct resource_loader* self, resource* resource) {
    if (!self || !resource) {
        CWARN("binary_loader_unload called with nullptr for self or resource.");
        return;
    }
    ...
    if (resource->data) {
        cfree(resource->data, resource->data_size, MEMORY_TAG_ARRAY);
        resource->data = 0;
