...
static b8 import_fnt_file(file_handle* fnt_file, const char* out_cbf_filename, bitmap_font_resource_data* out_data);
static b8 read_cbf_file(file_handle* cbf_file, bitmap_font_resource_data* data);
static b8 write_cbf_file(const char* path, bitmap_font_resource_data* data);
...
#define SUPPORTED_FILETYPE_COUNT 2
    ...
    supported_filetypes[0] = (supported_bitmap_font_filetype){".cbf", BITMAP_FONT_FILE_TYPE_CBF, true};
    ...
    if (type == BITMAP_FONT_FILE_TYPE_NOT_FOUND) {
        CERROR("Unable to find bitmap font of supported type called '%s'.", name);
        return false;
    }
    ...
    switch (type) {
        case BITMAP_FONT_FILE_TYPE_FNT: {
            // Generate the CBF filename.
            char cbf_file_name[512];
            string_format(cbf_file_name, "%s/%s/%s%s", resource_system_base_path(), self->type_path, name, ".cbf");
            result = import_fnt_file(&f, cbf_file_name, &resource_data);
            break;
        }
        case BITMAP_FONT_FILE_TYPE_CBF:
            result = read_cbf_file(&f, &resource_data);
            break;
    ...
    if (!result) {
        CERROR("Failed to process bitmap font file '%s'.", full_file_path);
    ...
    out_resource->data = callocate(sizeof(bitmap_font_resource_data), MEMORY_TAG_RESOURCE);
    ccopy_memory(out_resource->data, &resource_data, sizeof(bitmap_font_resource_data));
    ...
    if (self && resource) {
        if (resource->data) {
            ...
            if (data->data.glyph_count && data->data.glyphs) {
                cfree(data->data.glyphs, sizeof(font_glyph) * data->data.glyph_count, MEMORY_TAG_ARRAY);
                data->data.glyphs = 0;
            }

            if (data->data.kerning_count && data->data.kernings) {
                cfree(data->data.kernings, sizeof(font_kerning) * data->data.kerning_count, MEMORY_TAG_ARRAY);
                data->data.kernings = 0;
            }

            if (data->page_count && data->pages) {
                cfree(data->pages, sizeof(bitmap_font_page) * data->page_count, MEMORY_TAG_ARRAY);
                data->pages = 0;
            }

            cfree(resource->data, sizeof(bitmap_font_resource_data), MEMORY_TAG_RESOURCE);
            resource->data = 0;
            ...
            if (resource->full_path) {
                ...
                cfree(resource->full_path, sizeof(char) * length, MEMORY_TAG_STRING);
...
#define VERIFY_LINE(line_type, line_num, expected, actual)                                                                                     \
    if (actual != expected) {                                                                                                                  \
        CERROR("Error in file format reading type '%s', line %u. Expected %d element(s) but read %d.", line_type, line_num, expected, actual); \
        return false;                                                                                                                          \
    }

static b8 import_fnt_file(file_handle* fnt_file, const char* out_cbf_filename, bitmap_font_resource_data* out_data) {
    czero_memory(out_data, sizeof(bitmap_font_resource_data));
    char line_buf[512] = "";
    char* p = &line_buf[0];
    u64 line_length = 0;
    u32 line_num = 0;
            ...
            case 'c': {
                ...
                if (line_buf[1] == 'o') {
                    ...
                    if (out_data->page_count > 0) {
                        if (!out_data->pages) {
                            out_data->pages = callocate(sizeof(bitmap_font_page) * out_data->page_count, MEMORY_TAG_ARRAY);
                        }
                    } else {
                        CERROR("Pages is 0, which should not be possible. Font file reading aborted.");
                        return false;
                    }
                    ...
                    if (line_buf[4] == 's') {
                        ...
                        if (out_data->data.glyph_count > 0) {
                            if (!out_data->data.glyphs) {
                                out_data->data.glyphs = callocate(sizeof(font_glyph) * out_data->data.glyph_count, MEMORY_TAG_ARRAY);
                            }
                        } else {
                            CERROR("Glyph count is 0, which should not be possible. Font file reading aborted.");
                            return false;
                        }
                    } else {
                        // Assume 'char' line
            ...
            case 'k': {
                ...
                if (line_buf[7] == 's') {
                    ...
                    if (!out_data->data.kernings) {
                        out_data->data.kernings = callocate(sizeof(font_kerning) * out_data->data.kerning_count, MEMORY_TAG_ARRAY);
                    }
                } else if (line_buf[7] == ' ') {
                    // Kerning record
    ...
    return write_cbf_file(out_cbf_filename, out_data);
}

static b8 read_cbf_file(file_handle* file, bitmap_font_resource_data* data) {
    czero_memory(data, sizeof(bitmap_font_resource_data));
    ...
    if (header.magic_number != RESOURCE_MAGIC && header.resource_type == RESOURCE_TYPE_BITMAP_FONT) {
        CERROR("CBF file header is invalid and cannot be read.");
    ...
    data->pages = callocate(sizeof(bitmap_font_page) * data->page_count, MEMORY_TAG_ARRAY);
    ...
    data->data.glyphs = callocate(sizeof(font_glyph) * data->data.glyph_count, MEMORY_TAG_ARRAY);
    ...
    if (data->data.kerning_count > 0) {
        // Allocate kernings array
        data->data.kernings = callocate(sizeof(font_glyph) * data->data.kerning_count, MEMORY_TAG_ARRAY);
...
static b8 write_cbf_file(const char* path, bitmap_font_resource_data* data) {
    ...
    if (!filesystem_open(path, FILE_MODE_WRITE, true, &file)) {
        CERROR("Failed to open file for writing: %s", path);
        return false;
    }
