...
#define IMAGE_EXTENSION_COUNT 4
    ...
    if (!found) {
        CERROR("Image resource loader failed find file '%s' or with any supported extension.", full_file_path);
        return false;
    }

    file_handle f;
    if (!filesystem_open(full_file_path, FILE_MODE_READ, true, &f)) {
        CERROR("Unable to read file: %s.", full_file_path);
        filesystem_close(&f);
        return false;
    }
    ...
    if (!filesystem_size(&f, &file_size)) {
        CERROR("Unable to get size of file: %s.", full_file_path);
        filesystem_close(&f);
        return false;
    }
    ...
    u8* raw_data = callocate(file_size, MEMORY_TAG_TEXTURE);
    if (!raw_data) {
        CERROR("Unable to read file '%s'.", full_file_path);
        filesystem_close(&f);
        return false;
    ...
    if (bytes_read != file_size) {
        CERROR("File size if %llu does not match expected: %llu", bytes_read, file_size);
        return false;
    }

    u8* data = stbi_load_from_memory(raw_data, file_size, &width, &height, &channel_count, required_channel_count);
    if (!data) {
        CERROR("Image resource loader failed to load file '%s'.", full_file_path);
        return false;
    }

    cfree(raw_data, file_size, MEMORY_TAG_TEXTURE);
