...
b8 material_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f)) {
        CERROR("material_loader_load - unable to open material file for reading: '%s'.", full_file_path);
        return false;
    }
...
void material_loader_unload(struct resource_loader* self, resource* resource) {
    if (!self || !resource) {
        CWARN("material_loader_unload called with nullptr for self or resource.");
        return;
    }
    ...
    if (resource->data) {
        cfree(resource->data, resource->data_size, MEMORY_TAG_MATERIAL_INSTANCE);
        resource->data = 0;
