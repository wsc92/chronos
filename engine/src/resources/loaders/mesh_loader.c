...
#define SUPPORTED_FILETYPE_COUNT 2
    ...
    if (type == MESH_FILE_TYPE_NOT_FOUND) {
        CERROR("Unable to find mesh of supported type called '%s'.", name);
        return false;
    }
    ...
    if (!result) {
        CERROR("Failed to process mesh file '%s'.", full_file_path);
   ...
   // Groups
        ...
        switch (first_char) {
            ...
            case 'g': {
                ...
                for (u64 i = 0; i < group_count; ++i) {
                    ...
                    czero_memory(material_names[i], 64);
                }
                current_mat_name_count = 0;
                darray_clear(groups);
                czero_memory(name, 512);
    ...
    if (string_length(material_file_name) > 0) {
        ...
        if (!import_obj_material_library_file(full_mtl_path)) {
            CERROR("Error reading obj mtl file.");
        }
    }
    ...
    for (u64 i = 0; i < count; ++i) {
        ...
        CDEBUG("Geometry de-duplication process starting on geometry object named '%s'...", g->name);
        ...
        u32* indices = callocate(sizeof(u32) * g->index_count, MEMORY_TAG_ARRAY);
        ccopy_memory(indices, g->indices, sizeof(u32) * g->index_count);
        // Destroy the darray
...
void process_subobject(vec3* positions, vec3* normals, vec2* tex_coords, mesh_face_data* faces, geometry_config* out_data) {
    ...
    b8 extent_set = false;
    czero_memory(&out_data->min_extents, sizeof(vec3));
    czero_memory(&out_data->max_extents, sizeof(vec3));
    ...
    if (normal_count == 0) {
        CWARN("No normals are present in this model.");
        skip_normals = true;
    }
    if (tex_coord_count == 0) {
        CWARN("No texture coordinates are present in this model.");
...
b8 import_obj_material_library_file(const char* mtl_file_path) {
    CDEBUG("Importing obj .mtl file '%s'...", mtl_file_path);
    ...
    if (!filesystem_open(mtl_file_path, FILE_MODE_READ, false, &mtl_file)) {
        CERROR("Unable to open mtl file: %s", mtl_file_path);
        return false;
    }

    material_config current_config;
    czero_memory(&current_config, sizeof(current_config));
