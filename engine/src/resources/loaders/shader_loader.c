...
b8 shader_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f)) {
        CERROR("shader_loader_load - unable to open shader file for reading: '%s'.", full_file_path);
        return false;
    }

    out_resource->full_path = string_duplicate(full_file_path);

    shader_config* resource_data = callocate(sizeof(shader_config), MEMORY_TAG_RESOURCE);
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (equal_index == -1) {
            CWARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui.", full_file_path, line_number);
            line_number++;
            continue;
        }
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            if (resource_data->stage_count == 0) {
                ...
                CERROR("shader_loader_load: Invalid file layout. Count mismatch between stage names and stage filenames.");
            }
            ...
            for (u8 i = 0; i < resource_data->stage_count; ++i) {
                if (strings_equali(stage_names[i], "frag") || strings_equali(stage_names[i], "fragment")) {
                    ...
                    CERROR("shader_loader_load: Invalid file layout. Unrecognized stage '%s'", stage_names[i]);
                }
            }
            ...
            if (field_count != 2) {
                CERROR("shader_loader_load: Invalid file layout. Attribute fields must be 'type,name'. Skipping.");
            } else {
                ...
                if (strings_equali(fields[0], "f32")) {
                    ...
                    CERROR("shader_loader_load: Invalid file layout. Attribute type must be f32, vec2, vec3, vec4, i8, i16, i32, u8, u16, or u32.");
                    CWARN("Defaulting to f32.");
                    attribute.type = SHADER_ATTRIB_TYPE_FLOAT32;
                    attribute.size = 4;
                }
            ...
            u32 field_count = string_split(trimmed_value, ',', &fields, true, true);
            if (field_count != 3) {
                CERROR("shader_loader_load: Invalid file layout. Uniform fields must be 'type,scope,name'. Skipping.");
            } else {
                ...
                if (strings_equali(fields[0], "f32")) {
                    ...
                    CERROR("shader_loader_load: Invalid file layout. Uniform type must be f32, vec2, vec3, vec4, i8, i16, i32, u8, u16, u32 or mat4.");
                    CWARN("Defaulting to f32.");
                    uniform.type = SHADER_UNIFORM_TYPE_FLOAT32;
                    uniform.size = 4;
                }
                ...
                if (strings_equal(fields[1], "0")) {
                    ...
                    CERROR("shader_loader_load: Invalid file layout: Uniform scope must be 0 for global, 1 for instance or 2 for local.");
                    CWARN("Defaulting to global.");
                    uniform.scope = SHADER_SCOPE_GLOBAL;
                }
...
void shader_loader_unload(struct resource_loader* self, resource* resource) {
    ...
    for (u32 i = 0; i < count; ++i) {
        ...
        cfree(data->attributes[i].name, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    }
    ...
    for (u32 i = 0; i < count; ++i) {
        ...
        cfree(data->uniforms[i].name, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    }
    darray_destroy(data->uniforms);

    cfree(data->renderpass_name, sizeof(char) * (string_length(data->renderpass_name) + 1), MEMORY_TAG_STRING);
    cfree(data->name, sizeof(char) * (string_length(data->name) + 1), MEMORY_TAG_STRING);
    czero_memory(data, sizeof(shader_config));

    if (!resource_unload(self, resource, MEMORY_TAG_RESOURCE)) {
        CWARN("shader_loader_unload called with nullptr for self or resource.");
    }
}
