...
b8 import_fontconfig_file(file_handle* f, const char* type_path, const char* out_csf_filename, system_font_resource_data* out_resource);
b8 read_csf_file(file_handle* file, system_font_resource_data* data);
b8 write_csf_file(const char* out_csf_filename, system_font_resource_data* resource);
...
#define SUPPORTED_FILETYPE_COUNT 2
    ...
    supported_filetypes[0] = (supported_system_font_filetype){".csf", SYSTEM_FONT_FILE_TYPE_CSF, true};
    ...
    if (type == SYSTEM_FONT_FILE_TYPE_NOT_FOUND) {
        CERROR("Unable to find system font of supported type called '%s'.", name);
        return false;
    }
    ...
    switch (type) {
        case SYSTEM_FONT_FILE_TYPE_FONTCONFIG: {
            // Generate the csf filename.
            char csf_file_name[512];
            string_format(csf_file_name, "%s/%s/%s%s", resource_system_base_path(), self->type_path, name, ".csf");
            result = import_fontconfig_file(&f, self->type_path, csf_file_name, &resource_data);
            break;
        }
        case SYSTEM_FONT_FILE_TYPE_CSF:
            result = read_csf_file(&f, &resource_data);
            break;
    ...
    if (!result) {
        CERROR("Failed to process system font file '%s'.", full_file_path);
        out_resource->data = 0;
        out_resource->data_size = 0;
        return false;
    }

    out_resource->data = callocate(sizeof(system_font_resource_data), MEMORY_TAG_RESOURCE);
    ccopy_memory(out_resource->data, &resource_data, sizeof(system_font_resource_data));
...
b8 import_fontconfig_file(file_handle* f, const char* type_path, const char* out_csf_filename, system_font_resource_data* out_resource) {
    ...
    while (filesystem_read_line(f, 511, &p, &line_length)) {
        ...
        if (equal_index == -1) {
            CWARN("Potential formatting issue found in file: '=' token not found. Skipping line %u.", line_number);
            line_number++;
            continue;
        }
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            if (!filesystem_open(full_file_path, FILE_MODE_READ, true, &font_binary_handle)) {
                CERROR("Unable to open binary font file. Load process failed.");
                return false;
            }
            u64 file_size;
            if (!filesystem_size(&font_binary_handle, &file_size)) {
                CERROR("Unable to get file size of binary font file. Load process failed.");
                return false;
            }
            out_resource->font_binary = callocate(file_size, MEMORY_TAG_RESOURCE);
            if (!filesystem_read_all_bytes(&font_binary_handle, out_resource->font_binary, &out_resource->binary_size)) {
                CERROR("Unable to perform binary read on font file. Load process failed.");
                return false;
            }

            // Might still work anyway, so continue.
            if (out_resource->binary_size != file_size) {
                CWARN("Mismatch between filesize and bytes read in font file. File may be corrupt.");
            }
        ...
        czero_memory(line_buf, sizeof(char) * 512);
        line_number++;
    }
    ...
    if (!out_resource->font_binary || darray_length(out_resource->fonts) < 1) {
        CERROR("Font configuration did not provide a binary and at least one font face. Load process failed.");
        return false;
    }

    return write_csf_file(out_csf_filename, out_resource);
}

b8 read_csf_file(file_handle* file, system_font_resource_data* data) {
    czero_memory(data, sizeof(system_font_resource_data));
    ...
    if (header.magic_number != RESOURCE_MAGIC && header.resource_type == RESOURCE_TYPE_SYSTEM_FONT) {
        CERROR("CSF file header is invalid and cannot be read.");
...
void system_font_loader_unload(struct resource_loader* self, resource* resource) {
    if (self && resource) {
        ...
        if (data->font_binary) {
            cfree(data->font_binary, data->binary_size, MEMORY_TAG_RESOURCE);
            data->font_binary = 0;
            data->binary_size = 0;
        }
    }
