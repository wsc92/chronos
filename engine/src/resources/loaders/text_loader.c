...
b8 text_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f)) {
        CERROR("text_loader_load - unable to open file for text reading: '%s'.", full_file_path);
        return false;
    }

    u64 file_size = 0;
    if (!filesystem_size(&f, &file_size)) {
        CERROR("Unable to text read file: %s.", full_file_path);
        filesystem_close(&f);
        return false;
    }

    // TODO: Should be using an allocator here.
    char* resource_data = callocate(sizeof(char) * file_size, MEMORY_TAG_ARRAY);
    u64 read_size = 0;
...
void text_loader_unload(struct resource_loader* self, resource* resource) {
    if (!self || !resource) {
        CWARN("text_loader_unload called with nullptr for self or resource.");
        return;
    }
