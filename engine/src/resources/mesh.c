...
void mesh_load_job_success(void* params) {
    ...
    mesh_params->out_mesh->geometries = callocate(sizeof(geometry*) * mesh_params->out_mesh->geometry_count, MEMORY_TAG_ARRAY);
    ...
    CTRACE("Successfully loaded mesh '%s'.", mesh_params->resource_name);
...
void mesh_load_job_fail(void* params) {
    ...
    CERROR("Failed to load mesh '%s'.", mesh_params->resource_name);
...
b8 mesh_load_job_start(void* params, void* result_data) {
    ...
    ccopy_memory(result_data, load_params, sizeof(mesh_load_params));
...
void mesh_unload(mesh* m) {
    if (m) {
        ...
        cfree(m->geometries, sizeof(geometry*) * m->geometry_count, MEMORY_TAG_ARRAY);
        czero_memory(m, sizeof(mesh));
