...
b8 ui_text_create(ui_text_type type, const char* font_name, u16 font_size, const char* text_content, ui_text* out_text) {
    if (!font_name || !text_content || !out_text) {
        CERROR("ui_text_create requires a valid pointer to font_name, text_content and out_text");
        return false;
    }

    czero_memory(out_text, sizeof(ui_text));
    ...
    if (!font_system_acquire(font_name, font_size, out_text)) {
        CERROR("Unable to acquire font: '%s'. ui_text cannot be created.", font_name);
        return false;
    }
    ...
    if (!renderer_shader_acquire_instance_resources(ui_shader, font_maps, &out_text->instance_id)) {
        CFATAL("Unable to acquire shader resources for font texture map.");
        return false;
    }

    // Generate the vertex buffer.
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_VERTEX, text_length * quad_size, false, &out_text->vertex_buffer)) {
        CERROR("ui_text_create failed to create vertex renderbuffer.");
        return false;
    }
    if (!renderer_renderbuffer_bind(&out_text->vertex_buffer, 0)) {
        CERROR("ui_text_create failed to bind vertex renderbuffer.");
        return false;
    }
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_INDEX, text_length * quad_index_size, false, &out_text->index_buffer)) {
        CERROR("ui_text_create failed to create index renderbuffer.");
        return false;
    }
    if (!renderer_renderbuffer_bind(&out_text->index_buffer, 0)) {
        CERROR("ui_text_create failed to bind index renderbuffer.");
        return false;
    }

    // Verify atlas has the glyphs needed.
    if (!font_system_verify_atlas(out_text->data, text_content)) {
        CERROR("Font atlas verification failed.");
        return false;
    }
...
void ui_text_destroy(ui_text* text) {
    if (text) {
        ...
        if (text->text) {
            ...
            cfree(text->text, sizeof(char) * text_length, MEMORY_TAG_STRING);
            text->text = 0;
        }
        ...
        if (!renderer_shader_release_instance_resources(ui_shader, text->instance_id)) {
            CFATAL("Unable to release shader resources for font texture map.");
        }
    }
    czero_memory(text, sizeof(ui_text));
}
...
void ui_text_set_text(ui_text* u_text, const char* text) {
    if (u_text && u_text->text) {
        ...
        cfree(u_text->text, sizeof(char) * text_length, MEMORY_TAG_STRING);
...
void regenerate_geometry(ui_text* text) {
    ...
    if (vertex_buffer_size > text->vertex_buffer.total_size) {
        if (!renderer_renderbuffer_resize(&text->vertex_buffer, vertex_buffer_size)) {
            CERROR("regenerate_geometry for ui text failed to resize vertex renderbuffer.");
            return;
        }
    }
    ...
    if (index_buffer_size > text->index_buffer.total_size) {
        if (!renderer_renderbuffer_resize(&text->index_buffer, index_buffer_size)) {
            CERROR("regenerate_geometry for ui text failed to resize index renderbuffer.");
            return;
        }
    }
    ...
    vertex_2d* vertex_buffer_data = callocate(vertex_buffer_size, MEMORY_TAG_ARRAY);
    u32* index_buffer_data = callocate(index_buffer_size, MEMORY_TAG_ARRAY);
    ...
    for (u32 c = 0, uc = 0; c < char_length; ++c) {
        ...
        if (!bytes_to_codepoint(text->text, c, &codepoint, &advance)) {
            CWARN("Invalid UTF-8 found in string, using unknown codepoint of -1");
            codepoint = -1;
        }
        ...
        if (g) {
            ...
            CERROR("Unable to find unknown codepoint. Skipping.");
            // Increment utf-8 character count.
            uc++;
            continue;
        }
    ...
    // Clean up.
    cfree(vertex_buffer_data, vertex_buffer_size, MEMORY_TAG_ARRAY);
    cfree(index_buffer_data, index_buffer_size, MEMORY_TAG_ARRAY);

    // Verify results.
    if (!vertex_load_result) {
        CERROR("regenerate_geometry failed to load data into vertex buffer range.");
    }
    if (!index_load_result) {
        CERROR("regenerate_geometry failed to load data into index buffer range.");
    }
}
