...
b8 font_system_initialize(u64* memory_requirement, void* memory, font_system_config* config) {
    if (config->max_bitmap_font_count == 0 || config->max_system_font_count == 0) {
        CFATAL("font_system_initialize - config.max_bitmap_font_count and config.max_system_font_count must be > 0.");
        return false;
    }
    ...
    *memory_requirement = struct_requirement + bmp_array_requirement + sys_array_requirement + bmp_hashtable_requirement + sys_hashtable_requirement;
    ...
    for (u32 i = 0; i < state_ptr->config.default_bitmap_font_count; ++i) {
        if (!font_system_load_bitmap_font(&state_ptr->config.bitmap_font_configs[i])) {
            CERROR("Failed to load bitmap font: %s", state_ptr->config.bitmap_font_configs[i].name);
        }
    }
    // System fonts.
    for (u32 i = 0; i < state_ptr->config.default_system_font_count; ++i) {
        if (!font_system_load_system_font(&state_ptr->config.system_font_configs[i])) {
            CERROR("Failed to load system font: %s", state_ptr->config.system_font_configs[i].name);
        }
    }
...
b8 font_system_load_system_font(system_font_config* config) {
    ...
    if (!resource_system_load(config->resource_name, RESOURCE_TYPE_SYSTEM_FONT, 0, &loaded_resource)) {
        CERROR("Failed to load system font.");
        return false;
    }
    ...
    for (u32 i = 0; i < font_face_count; ++i) {
        ...
        if (!hashtable_get(&state_ptr->system_font_lookup, face->name, &id)) {
            CERROR("Hashtable lookup failed. Font will not be loaded.");
            return false;
        }
        if (id != INVALID_ID_U16) {
            CWARN("A font named '%s' already exists and will not be loaded again.", config->name);
        ...
        if (id == INVALID_ID_U16) {
            CERROR("No space left to allocate a new font. Increase maximum number allowed in font system config.");
            return false;
        }
        ...
        if (result == 0) {
            ...
            CERROR("Failed to init system font %s at index %i.", loaded_resource.full_path, i);
            return false;
        }
        ...
        if (!create_system_font_variant(lookup, config->default_size, face->name, &variant)) {
            CERROR("Failed to create variant: %s, index %i", face->name, i);
            continue;
        }

        // Also perform setup for the variant
        if (!setup_font_data(&variant)) {
            CERROR("Failed to setup font data");
            continue;
        }
        ...
        if (!hashtable_set(&state_ptr->system_font_lookup, face->name, &id)) {
            CERROR("Hashtable set failed on font load.");
            return false;
        }
    }
...
b8 font_system_load_bitmap_font(bitmap_font_config* config) {
    ...
    if (id == INVALID_ID_U16) {
        CERROR("No space left to allocate a new bitmap font. Increase maximum number allowed in font system config.");
        return false;
    }
    ...
    if (!resource_system_load(config->resource_name, RESOURCE_TYPE_BITMAP_FONT, 0, &lookup->font.loaded_resource)) {
        CERROR("Failed to load bitmap font.");
        return false;
    }
...
b8 font_system_acquire(const char* font_name, u16 font_size, struct ui_text* text) {
    if (text->type == UI_TEXT_TYPE_BITMAP) {
        ...
        if (!hashtable_get(&state_ptr->bitmap_font_lookup, font_name, &id)) {
            CERROR("Bitmap font lookup failed on acquire.");
            return false;
        }

        if (id == INVALID_ID_U16) {
            CERROR("A bitmap font named '%s' was not found. Font acquisition failed.", font_name);
            return false;
        }
        ...
        if (!hashtable_get(&state_ptr->system_font_lookup, font_name, &id)) {
            CERROR("System font lookup failed on acquire.");
            return false;
        }

        if (id == INVALID_ID_U16) {
            CERROR("A system font named '%s' was not found. Font acquisition failed.", font_name);
            return false;
        }
        ...
        if (!create_system_font_variant(lookup, font_size, font_name, &variant)) {
            CERROR("Failed to create variant: %s, index %i, size %i", lookup->face, lookup->index, font_size);
            return false;
        }
    ...
    CERROR("Unrecognized font type: %d", text->type);
    return false;
}
...
b8 font_system_verify_atlas(font_data* font, const char* text) {
    if (font->type == FONT_TYPE_BITMAP) {
        ...
        if (id == INVALID_ID_U16) {
            CERROR("A system font named '%s' was not found. Font atlas verification failed.", font->face);
            return false;
        }
    ...
    CERROR("font_system_verify_atlas failed: Unknown font type.");
    return false;
}
...
b8 setup_font_data(font_data* font) {
    ...
    if (!renderer_texture_map_acquire_resources(&font->atlas)) {
        CERROR("Unable to acquire resources for font atlas texture map.");
        return false;
    }
...
b8 create_system_font_variant(system_font_lookup* lookup, u16 size, const char* font_name, font_data* out_variant) {
    czero_memory(out_variant, sizeof(font_data));
    ...
    out_variant->internal_data = callocate(out_variant->internal_data_size, MEMORY_TAG_SYSTEM_FONT);
...
b8 rebuild_system_font_variant_atlas(system_font_lookup* lookup, font_data* variant) {
    ...
    u8* pixels = callocate(pack_image_size, MEMORY_TAG_ARRAY);
    u32 codepoint_count = darray_length(internal_data->codepoints);
    stbtt_packedchar* packed_chars = callocate(sizeof(stbtt_packedchar) * codepoint_count, MEMORY_TAG_ARRAY);
    ...
    if (!stbtt_PackFontRanges(&context, lookup->font_binary, lookup->index, &range, 1)) {
        CERROR("stbtt_PackFontRanges failed");
        return false;
    }
    ...
    u8* rgba_pixels = callocate(pack_image_size * 4, MEMORY_TAG_ARRAY);
    ...
    cfree(pixels, pack_image_size, MEMORY_TAG_ARRAY);
    cfree(rgba_pixels, pack_image_size * 4, MEMORY_TAG_ARRAY);

    // Regenerate glyphs
    if (variant->glyphs && variant->glyph_count) {
        cfree(variant->glyphs, sizeof(font_glyph) * variant->glyph_count, MEMORY_TAG_ARRAY);
    }
    variant->glyph_count = codepoint_count;
    variant->glyphs = callocate(sizeof(font_glyph) * codepoint_count, MEMORY_TAG_ARRAY);
    ...
    if (variant->kernings && variant->kerning_count) {
        cfree(variant->kernings, sizeof(font_kerning) * variant->kerning_count, MEMORY_TAG_ARRAY);
    }
    variant->kerning_count = stbtt_GetKerningTableLength(&lookup->info);
    if (variant->kerning_count) {
        variant->kernings = callocate(sizeof(font_kerning) * variant->kerning_count, MEMORY_TAG_ARRAY);
        // Get the kerning table for the current font.
        stbtt_kerningentry* kerning_table = callocate(sizeof(stbtt_kerningentry) * variant->kerning_count, MEMORY_TAG_ARRAY);
        ...
        if (entry_count != variant->kerning_count) {
            CERROR("Kerning entry count mismatch: %i->%i", entry_count, variant->kerning_count);
            return false;
        }
...
b8 verify_system_font_size_variant(system_font_lookup* lookup, font_data* variant, const char* text) {
    ...
    for (u32 i = 0; i < char_length;) {
        ...
        if (!bytes_to_codepoint(text, i, &codepoint, &advance)) {
            CERROR("bytes_to_codepoint failed to get codepoint.");
            ++i;
            continue;
        } else {
