...
b8 geometry_system_initialize(u64* memory_requirement, void* state, geometry_system_config config) {
    if (config.max_geometry_count == 0) {
        CFATAL("geometry_system_initialize - config.max_geometry_count must be > 0.");
        return false;
    }
    ...
    *memory_requirement = struct_requirement + array_requirement;
    ...
    if (!create_default_geometries(state_ptr)) {
        CFATAL("Failed to create default geometries. Application cannot continue.");
        return false;
    }
...
geometry* geometry_system_acquire_by_id(u32 id) {
    ...
    CERROR("geometry_system_acquire_by_id cannot load invalid geometry id. Returning nullptr.");
    return 0;
}
...
geometry* geometry_system_acquire_from_config(geometry_config config, b8 auto_release) {
    ...
    if (!g) {
        CERROR("Unable to obtain free slot for geometry. Adjust configuration to allow more space. Returning nullptr.");
        return 0;
    }

    if (!create_geometry(state_ptr, config, g)) {
        CERROR("Failed to create geometry. Returning nullptr.");
        return 0;
    }
...
void geometry_system_release(geometry* geometry) {
    if (geometry && geometry->id != INVALID_ID) {
        ...
        if (ref->geometry.id == id) {
            ...
            CFATAL("Geometry id mismatch. Check registration logic, as this should never occur.");
        }
        return;
    }

    CWARN("geometry_system_release cannot release invalid geometry id. Nothing was done.");
}
...
geometry* geometry_system_get_default() {
    ...
    CFATAL("geometry_system_get_default called before system was initialized. Returning nullptr.");
    return 0;
}
...
geometry* geometry_system_get_default_2d() {
    ...
    CFATAL("geometry_system_get_default_2d called before system was initialized. Returning nullptr.");
    return 0;
}
...
b8 create_default_geometries(geometry_system_state* state) {
    ...
    if (!renderer_create_geometry(&state->default_geometry, sizeof(vertex_3d), 4, verts, sizeof(u32), 6, indices)) {
        CFATAL("Failed to create default geometry. Application cannot continue.");
        return false;
    }
    ...
    if (!renderer_create_geometry(&state->default_2d_geometry, sizeof(vertex_2d), 4, verts2d, sizeof(u32), 6, indices2d)) {
        CFATAL("Failed to create default 2d geometry. Application cannot continue.");
        return false;
    }
...
geometry_config geometry_system_generate_plane_config(f32 width, f32 height, u32 x_segment_count, u32 y_segment_count, f32 tile_x, f32 tile_y, const char* name, const char* material_name) {
    if (width == 0) {
        CWARN("Width must be nonzero. Defaulting to one.");
        width = 1.0f;
    }
    if (height == 0) {
        CWARN("Height must be nonzero. Defaulting to one.");
        height = 1.0f;
    }
    if (x_segment_count < 1) {
        CWARN("x_segment_count must be a positive number. Defaulting to one.");
        x_segment_count = 1;
    }
    if (y_segment_count < 1) {
        CWARN("y_segment_count must be a positive number. Defaulting to one.");
        y_segment_count = 1;
    }

    if (tile_x == 0) {
        CWARN("tile_x must be nonzero. Defaulting to one.");
        tile_x = 1.0f;
    }
    if (tile_y == 0) {
        CWARN("tile_y must be nonzero. Defaulting to one.");
        tile_y = 1.0f;
    }
...
geometry_config geometry_system_generate_cube_config(f32 width, f32 height, f32 depth, f32 tile_x, f32 tile_y, const char* name, const char* material_name) {
    ...
    if (depth == 0) {
        CWARN("Depth must be nonzero. Defaulting to one.");
        depth = 1;
    }
    if (tile_x == 0) {
        CWARN("tile_x must be nonzero. Defaulting to one.");
    ...
    ccopy_memory(config.vertices, verts, config.vertex_size * config.vertex_count);
