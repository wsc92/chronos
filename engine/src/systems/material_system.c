...
b8 material_system_initialize(u64* memory_requirement, void* state, material_system_config config) {
    if (config.max_material_count == 0) {
        CFATAL("material_system_initialize - config.max_material_count must be > 0.");
        return false;
    }
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    if (!create_default_material(state_ptr)) {
        CFATAL("Failed to create default material. Application cannot continue.");
        return false;
    }
...
material* material_system_acquire_from_config(material_config config) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_material_table, config.name, &ref)) {
        ...
        if (ref.handle == INVALID_ID) {
            ...
            if (!m || ref.handle == INVALID_ID) {
                CFATAL("material_system_acquire - Material system cannot hold anymore materials. Adjust configuration to allow more.");
                return 0;
            }

            // Create new material.
            if (!load_material(config, m)) {
                CERROR("Failed to load material '%s'.", config.name);
                return 0;
            }
            ...
            m->id = ref.handle;
            CTRACE("Material '%s' does not yet exist. Created, and ref_count is now %i.", config.name, ref.reference_count);
        } else {
            CTRACE("Material '%s' already exists, ref_count increased to %i.", config.name, ref.reference_count);
        }
    ...
    CERROR("material_system_acquire_from_config failed to acquire material '%s'. Null pointer will be returned.", config.name);
    return 0;
}
...
void material_system_release(const char* name) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_material_table, name, &ref)) {
        if (ref.reference_count == 0) {
            CWARN("Tried to release non-existent material: '%s'", name);
            return;
        }
        ref.reference_count--;
        if (ref.reference_count == 0 && ref.auto_release) {
            ...
            CTRACE("Released material '%s'., Material unloaded because reference count=0 and auto_release=true.", name);
        } else {
            CTRACE("Released material '%s', now has a reference count of '%i' (auto_release=%s).", name, ref.reference_count, ref.auto_release ? "true" : "false");
        }

        // Update the entry.
        hashtable_set(&state_ptr->registered_material_table, name, &ref);
    } else {
        CERROR("material_system_release failed to release material '%s'.", name);
    }
}
...
material* material_system_get_default() {
    ...
    CFATAL("material_system_get_default called before system is initialized.");
    return 0;
}

#define MATERIAL_APPLY_OR_FAIL(expr)                  \
    if (!expr) {                                      \
        CERROR("Failed to apply material: %s", expr); \
        return false;                                 \
    }
...
b8 material_system_apply_global(u32 shader_id, u64 renderer_frame_number, const mat4* projection, const mat4* view, const vec4* ambient_colour, const vec3* view_position, u32 render_mode) {
    ...
    if (shader_id == state_ptr->material_shader_id) {
        ...
        CERROR("material_system_apply_global(): Unrecognized shader id '%d' ", shader_id);
        return false;
    }
...
b8 material_system_apply_instance(material* m, b8 needs_update) {
    ...
    if (needs_update) {
        if (m->shader_id == state_ptr->material_shader_id) {
            ...
            CERROR("material_system_apply_instance(): Unrecognized shader id '%d' on shader '%s'.", m->shader_id, m->name);
            return false;
        }
    }
...
b8 material_system_apply_local(material* m, const mat4* model) {
    ...
    CERROR("Unrecognized shader id '%d'", m->shader_id);
    return false;
}
...
b8 load_material(material_config config, material* m) {
    ...
    if (!renderer_texture_map_acquire_resources(&m->diffuse_map)) {
        CERROR("Unable to acquire resources for diffuse texture map.");
        return false;
    }
    if (string_length(config.diffuse_map_name) > 0) {
        ...
        if (!m->diffuse_map.texture) {
            ...
            CWARN("Unable to load texture '%s' for material '%s', using default.", config.diffuse_map_name, m->name);
    ...
    if (!renderer_texture_map_acquire_resources(&m->specular_map)) {
        CERROR("Unable to acquire resources for specular texture map.");
        return false;
    }
    if (string_length(config.specular_map_name) > 0) {
        ...
        if (!m->specular_map.texture) {
            CWARN("Unable to load specular texture '%s' for material '%s', using default.", config.specular_map_name, m->name);
    ...
    if (!renderer_texture_map_acquire_resources(&m->normal_map)) {
        CERROR("Unable to acquire resources for normal texture map.");
        return false;
    }
    if (string_length(config.normal_map_name) > 0) {
        ...
        if (!m->normal_map.texture) {
            CWARN("Unable to load normal texture '%s' for material '%s', using default.", config.normal_map_name, m->name);
    ...
    if (!s) {
        CERROR("Unable to load material because its shader was not found: '%s'. This is likely a problem with the material asset.", config.shader_name);
        return false;
    }
    ...
    if (!renderer_shader_acquire_instance_resources(s, maps, &m->internal_id)) {
        CERROR("Failed to acquire renderer resources for material '%s'.", m->name);
        return false;
    }
...
void destroy_material(material* m) {
    CTRACE("Destroying material '%s'...", m->name);
...
b8 create_default_material(material_system_state* state) {
    ...
    if (!renderer_shader_acquire_instance_resources(s, maps, &state->default_material.internal_id)) {
        CFATAL("Failed to acquire renderer resources for default material. Application cannot continue.");
        return false;
    }
