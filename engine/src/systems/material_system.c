...
material* material_system_acquire(const char* name) {
    ...
    string_format(full_file_path, format_str, name, "cmt");
    if (!load_configuration_file(full_file_path, &config)) {
        CERROR("Failed to load material file: '%s'. Null pointer will be returned.", full_file_path);
        return 0;
    }
...
b8 create_default_material(material_system_state* state) {
    ...
    if (!renderer_create_material(&state->default_material)) {
        CFATAL("Failed to acquire renderer resources for default texture. Application cannot continue.");
        return false;
    }
...
b8 load_configuration_file(const char* path, material_config* out_config) {
    ...
    if (!filesystem_open(path, FILE_MODE_READ, false, &f)) {
        CERROR("load_configuration_file - unable to open material file for reading: '%s'.", path);
        return false;
    }
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (equal_index == -1) {
            CWARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui.", path, line_number);
            line_number++;
            continue;
        }
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            if (!string_to_vec4(trimmed_value, &out_config->diffuse_colour)) {
                CWARN("Error parsing diffuse_colour in file '%s'. Using default of white instead.", path);
