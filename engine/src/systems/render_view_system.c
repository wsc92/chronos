...
b8 render_view_system_initialize(u64* memory_requirement, void* state, render_view_system_config config) {
    if (config.max_view_count == 0) {
        CFATAL("render_view_system_initialize - config.max_view_count must be > 0.");
        return false;
    }
...
b8 render_view_system_create(const render_view_config* config) {
    if (!config) {
        CERROR("render_view_system_create requires a pointer to a valid config.");
        return false;
    }
    ...
    if (id != INVALID_ID_U16) {
        CERROR("render_view_system_create - A view named '%s' already exists. A new one will not be created.", config->name);
        return false;
    }
    ...
    if (id == INVALID_ID_U16) {
        CERROR("render_view_system_create - No available space for a new view. Change system config to account for more.");
        return false;
    }
    ...
    if (!view->on_create(view)) {
        CERROR("Failed to create view.");
        cfree(view->passes, sizeof(renderpass*) * view->renderpass_count, MEMORY_TAG_ARRAY);
        czero_memory(&state_ptr->registered_views[id], sizeof(render_view));
        return false;
    }
            ...
            for (u32 a = 0; a < target->attachment_count; ++a) {
                ...
                if (attachment->source == RENDER_TARGET_ATTACHMENT_SOURCE_DEFAULT) {
                    if (attachment->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
                        ...
                        CFATAL("Unsupported attachment type: 0x%x", attachment->type);
                        continue;
                    }
                } else if (attachment->source == RENDER_TARGET_ATTACHMENT_SOURCE_VIEW) {
                    if (!view->regenerate_attachment_target) {
                        CFATAL("RENDER_TARGET_ATTACHMENT_SOURCE_VIEW configured for an attachment whose view does not support this operation.");
                        continue;
                    } else {
                        if (!view->regenerate_attachment_target(view, r, attachment)) {
                            CERROR("View failed to regenerate attachment target for attachment type: 0x%x", attachment->type);
                        }
                    }
                }
            }
