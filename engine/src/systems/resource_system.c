...
b8 resource_system_initialize(u64* memory_requirement, void* state, void* config) {
    ...
    if (typed_config->max_loader_count == 0) {
        CFATAL("resource_system_initialize failed because config.max_loader_count==0.");
        return false;
    }
    ...
    *memory_requirement = sizeof(resource_system_state) + (sizeof(resource_loader) * typed_config->max_loader_count);
    ...
    CINFO("Resource system initialized with base path '%s'.", typed_config->asset_base_path);
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            if (l->id != INVALID_ID) {
                if (l->type == loader.type) {
                    CERROR("resource_system_register_loader - Loader of type %d already exists and will not be registered.", loader.type);
                    return false;
                } else if (loader.custom_type && string_length(loader.custom_type) > 0 && strings_equali(l->custom_type, loader.custom_type)) {
                    CERROR("resource_system_register_loader - Loader of custom type %s already exists and will not be registered.", loader.custom_type);
                    return false;
                }
            }
        }
...
b8 resource_system_load(const char* name, resource_type type, void* params, resource* out_resource) {
    ...
    CERROR("resource_system_load - No loader for type %d was found.", type);
    return false;
}
...
b8 resource_system_load_custom(const char* name, const char* custom_type, void* params, resource* out_resource) {
    ...
    out_resource->loader_id = INVALID_ID;
    CERROR("resource_system_load_custom - No loader for type %s was found.", custom_type);
    return false;
}
...
const char* resource_system_base_path() {
    ...
    CERROR("resource_system_base_path called before initialization, returning empty string.");
    return "";
}
