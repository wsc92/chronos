...
b8 shader_system_initialize(u64* memory_requirement, void* memory, shader_system_config config) {
    ...
    if (config.max_shader_count < 512) {
        if (config.max_shader_count == 0) {
            CERROR("shader_system_initialize - config.max_shader_count must be greater than 0");
            return false;
        } else {
            // This is to help avoid hashtable collisions.
            CWARN("shader_system_initialize - config.max_shader_count is recommended to be at least 512.");
        }
    }
...
void shader_system_shutdown(void* state) {
    if (state) {
        ...
        czero_memory(st, sizeof(shader_system_state));
    }
...
b8 shader_system_create(const shader_config* config) {
    ...
    czero_memory(out_shader, sizeof(shader));
    out_shader->id = id;
    if (out_shader->id == INVALID_ID) {
        CERROR("Unable to find free slot to create new shader. Aborting.");
        return false;
    }
    ...
    out_shader->hashtable_block = callocate(element_size * element_count, MEMORY_TAG_UNKNOWN);
    ...
    if (!renderer_renderpass_id(config->renderpass_name, &renderpass_id)) {
        CERROR("Unable to find renderpass '%s'", config->renderpass_name);
        return false;
    }
    ...
    if (!renderer_shader_initialize(out_shader)) {
        CERROR("shader_system_create: initialization failed for shader '%s'.", config->name);
...
void shader_destroy(shader* s) {
    ...
    for (u32 i = 0; i < sampler_count; ++i) {
        cfree(s->global_texture_maps[i], sizeof(texture_map), MEMORY_TAG_RENDERER);
    }
    ...
    if (s->name) {
        ...
        cfree(s->name, length + 1, MEMORY_TAG_STRING);
    }
    s->name = 0;
}
...
b8 shader_system_use_by_id(u32 shader_id) {
    ...
    if (state_ptr->current_shader_id != shader_id) {
        ...
        if (!renderer_shader_use(next_shader)) {
            CERROR("Failed to use shader '%s'.", next_shader->name);
            return false;
        }
        if (!renderer_shader_bind_globals(next_shader)) {
            CERROR("Failed to bind globals for shader '%s'.", next_shader->name);
            return false;
        }
    }
    return true;
...
u16 shader_system_uniform_index(shader* s, const char* uniform_name) {
    if (!s || s->id == INVALID_ID) {
        CERROR("shader_system_uniform_location called with invalid shader.");
        return INVALID_ID_U16;
    }

    u16 index = INVALID_ID_U16;
    if (!hashtable_get(&s->uniform_lookup, uniform_name, &index) || index == INVALID_ID_U16) {
        CERROR("Shader '%s' does not have a registered uniform named '%s'", s->name, uniform_name);
        return INVALID_ID_U16;
    }
...
b8 shader_system_uniform_set(const char* uniform_name, const void* value) {
    if (state_ptr->current_shader_id == INVALID_ID) {
        CERROR("shader_system_uniform_set called without a shader in use.");
        return false;
    }
...
b8 add_attribute(shader* shader, const shader_attribute_config* config) {
    ...
    switch (config->type) {
        ...
        case SHADER_ATTRIB_TYPE_FLOAT32_4:
            ...
            break;
        default:
            CERROR("Unrecognized type %d, defaulting to size of 4. This probably is not what is desired.");
            size = 4;
            break;
    }
...
b8 add_sampler(shader* shader, shader_uniform_config* config) {
    ...
    if (config->scope == SHADER_SCOPE_GLOBAL) {
        ...
        if (global_texture_count + 1 > state_ptr->config.max_global_textures) {
            CERROR("Shader global texture count %i exceeds max of %i", global_texture_count, state_ptr->config.max_global_textures);
            return false;
        }
        ...
        if (!renderer_texture_map_acquire_resources(&default_map)) {
            CERROR("Failed to acquire resources for global texture map during shader creation.");
            return false;
        }
        ...
        texture_map* map = callocate(sizeof(texture_map), MEMORY_TAG_RENDERER);
        *map = default_map;
        ...
        if (shader->instance_texture_count + 1 > state_ptr->config.max_instance_textures) {
            CERROR("Shader instance texture count %i exceeds max of %i", shader->instance_texture_count, state_ptr->config.max_instance_textures);
            return false;
        }
    ...
    if (!uniform_add(shader, config->name, 0, config->type, config->scope, location, true)) {
        CERROR("Unable to add sampler uniform.");
        return false;
    }
...
u32 get_shader_id(const char* shader_name) {
    ...
    if (!hashtable_get(&state_ptr->lookup, shader_name, &shader_id)) {
        CERROR("There is no shader registered named '%s'.", shader_name);
        return INVALID_ID;
    }
    return shader_id;
}
...
b8 uniform_add(shader* shader, const char* uniform_name, u32 size, shader_uniform_type type, shader_scope scope, u32 set_location, b8 is_sampler) {
    ...
    if (uniform_count + 1 > state_ptr->config.max_uniform_count) {
        CERROR("A shader can only accept a combined maximum of %d uniforms and samplers at global, instance and local scopes.", state_ptr->config.max_uniform_count);
        return false;
    }
...
b8 uniform_name_valid(shader* shader, const char* uniform_name) {
    if (!uniform_name || !string_length(uniform_name)) {
        CERROR("Uniform name must exist.");
        return false;
    }
    u16 location;
    if (hashtable_get(&shader->uniform_lookup, uniform_name, &location) && location != INVALID_ID_U16) {
        CERROR("A uniform by the name '%s' already exists on shader '%s'.", uniform_name, shader->name);
        return false;
    }
    return true;
}
...
b8 shader_uniform_add_state_valid(shader* shader) {
    if (shader->state != SHADER_STATE_UNINITIALIZED) {
        CERROR("Uniforms may only be added to shaders before initialization.");
        return false;
    }
    return true;
}
