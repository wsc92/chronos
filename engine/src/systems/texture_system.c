...
texture* texture_system_acquire(const char* name, b8 auto_release) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        ...
        if (ref.handle == INVALID_ID) {
            ...
            if (!t || ref.handle == INVALID_ID) {
                CFATAL("texture_system_acquire - Texture system cannot hold anymore textures. Adjust configuration to allow more.");
                return 0;
            }
    ...
    CERROR("texture_system_acquire failed to acquire texture '%s'. Null pointer will be returned.", name);
    return 0;
}
...
void texture_system_release(const char* name) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        ...
        CERROR("texture_system_release failed to release texture '%s'.", name);
    }
}
...
texture* texture_system_get_default_texture() {
    ...
    CERROR("texture_system_get_default_texture called before texture system initialization! Null pointer returned.");
    return 0;
}
...
texture* texture_system_get_default_diffuse_texture() {
    ...
    CERROR("texture_system_get_default_diffuse_texture called before texture system initialization! Null pointer returned.");
    return 0;
}
