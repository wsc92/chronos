...
b8 texture_system_initialize(u64* memory_requirement, void* state, texture_system_config config) {
    if (config.max_texture_count == 0) {
        CFATAL("texture_system_initialize - config.max_texture_count must be > 0.");
        return false;
    }
...
texture* texture_system_acquire(const char* name, b8 auto_release) {
    ...
    if (strings_equali(name, DEFAULT_TEXTURE_NAME)) {
        CWARN("texture_system_acquire called for default texture. Use texture_system_get_default_texture for texture 'default'.");
    ...
    if (!process_texture_reference(name, TEXTURE_TYPE_2D, 1, auto_release, false, &id)) {
        CERROR("texture_system_acquire failed to obtain a new texture id.");
        return 0;
    }
...
texture* texture_system_acquire_cube(const char* name, b8 auto_release) {
    ...
    // TODO: Check against other default texture names?
    if (strings_equali(name, DEFAULT_TEXTURE_NAME)) {
        CWARN("texture_system_acquire_cube called for default texture. Use texture_system_get_default_texture for texture 'default'.");
        return &state_ptr->default_texture;
    }
    ...
    if (!process_texture_reference(name, TEXTURE_TYPE_CUBE, 1, auto_release, false, &id)) {
        CERROR("texture_system_acquire_cube failed to obtain a new texture id.");
        return 0;
    }
...
texture* texture_system_aquire_writeable(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency) {
    ...
    if (!process_texture_reference(name, TEXTURE_TYPE_2D, 1, false, true, &id)) {
        CERROR("texture_system_aquire_writeable failed to obtain a new texture id.");
        return 0;
    }
...
void texture_system_release(const char* name) {
    ...
    if (!process_texture_reference(name, TEXTURE_TYPE_2D, -1, false, false, &id)) {
        CERROR("texture_system_release failed to release texture '%s' properly.", name);
    }
}
...
void texture_system_wrap_internal(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency, b8 is_writeable, b8 register_texture, void* internal_data, texture* out_texture) {
    ...
    if (register_texture) {
        ...
        // resources are created and managed somewhere within the renderer internals.
        if (!process_texture_reference(name, TEXTURE_TYPE_2D, 1, false, true, &id)) {
            CERROR("texture_system_wrap_internal failed to obtain a new texture id.");
            return;
        }
        ...
        if (out_texture) {
            ...
            t = callocate(sizeof(texture), MEMORY_TAG_TEXTURE);
            // CTRACE("texture_system_wrap_internal created texture '%s', but not registering, resulting in an allocation. It is up to the caller to free this memory.", name);
        }
    }
...
b8 texture_system_resize(texture* t, u32 width, u32 height, b8 regenerate_internal_data) {
    if (t) {
        if (!(t->flags & TEXTURE_FLAG_IS_WRITEABLE)) {
            CWARN("texture_system_resize should not be called on textures that are not writeable.");
            return false;
        }
        t->width = width;
        t->height = height;
...
#define RETURN_TEXT_PTR_OR_NULL(texture, func_name)                                              \
    ...
    CERROR("%s called before texture system initialization! Null pointer returned.", func_name); \
    return 0;
...
b8 create_default_textures(texture_system_state* state) {
    ...
    cset_memory(diff_pixels, 255, sizeof(u8) * 16 * 16 * 4);
...
b8 load_cube_textures(const char* name, const char texture_names[6][TEXTURE_NAME_MAX_LENGTH], texture* t) {
    ...
    for (u8 i = 0; i < 6; ++i) {
        ...
        if (!resource_system_load(texture_names[i], RESOURCE_TYPE_IMAGE, &params, &img_resource)) {
            CERROR("load_cube_textures() - Failed to load image resource for texture '%s'", texture_names[i]);
            return false;
        }
        ...
        if (!pixels) {
            ...
            pixels = callocate(sizeof(u8) * image_size * 6, MEMORY_TAG_ARRAY);
        } else {
            ...
            if (t->width != resource_data->width || t->height != resource_data->height || t->channel_count != resource_data->channel_count) {
                CERROR("load_cube_textures - All textures must be the same resolution and bit depth.");
                cfree(pixels, sizeof(u8) * image_size * 6, MEMORY_TAG_ARRAY);
                pixels = 0;
                return false;
            }
        }
        ...
        ccopy_memory(pixels + image_size * i, resource_data->pixels, image_size);
...
void texture_load_job_success(void* params) {
    ...
    *texture_params->out_texture = texture_params->temp_texture;
    ...
    czero_memory(&old, sizeof(texture));
    ...
    CTRACE("Successfully loaded texture '%s'.", texture_params->resource_name);
    ...
    if (texture_params->resource_name) {
        ...
        cfree(texture_params->resource_name, sizeof(char) * length + 1, MEMORY_TAG_STRING);
...
void texture_load_job_fail(void* params) {
    ...
    CERROR("Failed to load texture '%s'.", texture_params->resource_name);
...
b8 texture_load_job_start(void* params, void* result_data) {
    ...
    ccopy_memory(result_data, load_params, sizeof(texture_load_params));
...
void destroy_texture(texture* t) {
    ...
    czero_memory(t->name, sizeof(char) * TEXTURE_NAME_MAX_LENGTH);
...
b8 process_texture_reference(const char* name, texture_type type, i8 reference_diff, b8 auto_release, b8 skip_load, u32* out_texture_id) {
    *out_texture_id = INVALID_ID;
    if (state_ptr) {
        ...
        if (hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
            ...
            if (ref.reference_count == 0 && reference_diff > 0) {
                if (reference_diff > 0) {
                    ...
                    if (ref.auto_release) {
                        CWARN("Tried to release non-existent texture: '%s'", name);
                        return false;
                    } else {
                        CWARN("Tried to release a texture where autorelease=false, but references was already 0.");
            ...
            if (reference_diff < 0) {
                ...
                if (ref.handle == INVALID_ID) {
                    ...
                    if (*out_texture_id == INVALID_ID) {
                        CFATAL("process_texture_reference - Texture system cannot hold anymore textures. Adjust configuration to allow more.");
                        return false;
                    } else {
                        texture* t = &state_ptr->registered_textures[ref.handle];
                        t->type = type;
                        ...
                        if (skip_load) {
                            ...
                            if (type == TEXTURE_TYPE_CUBE) {
                                ...
                                if (!load_cube_textures(name, texture_names, t)) {
                                    *out_texture_id = INVALID_ID;
                                    CERROR("Failed to load cube texture '%s'.", name);
                                    return false;
                                }
                            } else {
                                if (!load_texture(name, t)) {
                                    *out_texture_id = INVALID_ID;
                                    CERROR("Failed to load texture '%s'.", name);
                                    return false;
                                }
                            }
                            t->id = ref.handle;
                        }
                        // CTRACE("Texture '%s' does not yet exist. Created, and ref_count is now %i.", name, ref.reference_count);
                    }
                } else {
                    *out_texture_id = ref.handle;
                    // CTRACE("Texture '%s' already exists, ref_count increased to %i.", name, ref.reference_count);
                }
            }
        ...
        CERROR("process_texture_reference failed to acquire id for name '%s'. INVALID_ID returned.", name);
        return false;
    }

    CERROR("process_texture_reference called before texture system is initialized.");
    return false;
}
