    ...
    if (code == EVENT_CODE_KEY_PRESSED) {
        ...
        if (key_code == KEY_ENTER) {
            ...
            if (len > 0) {
                ...
                cfree(str, len + 1, MEMORY_TAG_STRING);
            }
        } else {
...
void debug_console_create() {
    if (!state_ptr) {
        state_ptr = callocate(sizeof(debug_console_state), MEMORY_TAG_GAME);
...
b8 debug_console_load() {
    ...
    if (!ui_text_create(UI_TEXT_TYPE_SYSTEM, "Noto Sans CJK JP", 31, "", &state_ptr->entry_control)) {
        CFATAL("Unable to create entry text control for debug console.");
        return false;
    }
...
void debug_console_update() {
    if (state_ptr && state_ptr->dirty) {
        ...
        u32 max_lines = CMIN(state_ptr->line_display_count, CMAX(line_count, state_ptr->line_display_count));

        // Calculate the min line first, taking into account the line offset as well.
        u32 min_line = CMAX(line_count - max_lines - state_ptr->line_offset, 0);
        u32 max_line = min_line + max_lines - 1;

        // Hopefully big enough to handle most things.
        char buffer[16384];
        czero_memory(buffer, sizeof(char) * 16384);
        u32 buffer_pos = 0;
...
void debug_console_move_up() {
    if (state_ptr) {
        ...
        state_ptr->line_offset = CMIN(state_ptr->line_offset, line_count - state_ptr->line_display_count);
    }
}
...
void debug_console_move_down() {
    if (state_ptr) {
        ...
        state_ptr->line_offset--;
        state_ptr->line_offset = CMAX(state_ptr->line_offset, 0);
    }
}
...
void debug_console_history_back() {
    if (state_ptr) {
        ...
        if (length > 0) {
            state_ptr->history_offset = CMIN(state_ptr->history_offset++, length - 1);
...
void debug_console_history_forward() {
    if (state_ptr) {
        u32 length = darray_length(state_ptr->history);
        if (length > 0) {
            state_ptr->history_offset = CMAX(state_ptr->history_offset--, 0);
            ui_text_set_text(&state_ptr->entry_control, state_ptr->history[length - state_ptr->history_offset - 1].command);
        }
    }
}
