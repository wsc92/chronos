...
#include "../../engine/src/core/metrics.h"
...
// TODO: temp
#include "../../engine/src/core/identifier.h"
#include "../../engine/src/math/transform.h"
#include "../../engine/src/resources/skybox.h"
#include "../../engine/src/resources/ui_text.h"
#include "../../engine/src/resources/mesh.h"
#include "../../engine/src/systems/geometry_system.h"
#include "../../engine/src/systems/material_system.h"
#include "../../engine/src/systems/render_view_system.h"
...
b8 game_boot(struct game* game_inst) {
    ...
    config->font_config.bitmap_font_configs = callocate(sizeof(bitmap_font_config) * 1, MEMORY_TAG_ARRAY);
    ...
    config->font_config.system_font_configs = callocate(sizeof(sys_font_config) * 1, MEMORY_TAG_ARRAY);
    ...
    if (!configure_render_views(config)) {
        CERROR("Failed to configure renderer views. Aborting application.");
        return false;
    }
...
b8 game_initialize(game* game_inst) {
    ...
    if (!skybox_create("skybox_cube", &state->sb)) {
        CERROR("Failed to create skybox, aborting game.");
        return false;
    }
    ...
    state->ui_meshes[0].geometries = callocate(sizeof(geometry*), MEMORY_TAG_ARRAY);
...
VSync: %s Drawn: %-5u Hovered: %s%u",
        ...
        state->last_update_elapsed * C_SEC_TO_US_MULTIPLIER,
        state->render_clock.elapsed * C_SEC_TO_US_MULTIPLIER,
        mouse_x, mouse_y,
        left_down ? "Y" : "N",
        right_down ? "Y" : "N",
        mouse_x_ndc,
