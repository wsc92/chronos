...
#include "../../engine/src/core/cvar.h"
...
void game_on_escape_callback(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    CDEBUG("game_on_escape_callback");
...
void game_print_memory_metrics(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    CINFO(usage);
    string_free(usage);
    CDEBUG("Allocations: %llu (%llu this frame)", state->alloc_count, state->alloc_count - state->prev_alloc_count);
}

static b8 game_on_cvar_changed(u16 code, void* sender, void* listener_inst, event_context data) {
    if (code == EVENT_CODE_CVAR_CHANGED && strings_equali(data.data.c, "vsync")) {
        toggle_vsync();
    }
    return false;
}
...
void game_setup_keymaps(application* game_inst) {
    event_register(EVENT_CODE_CVAR_CHANGED, 0, game_on_cvar_changed);
