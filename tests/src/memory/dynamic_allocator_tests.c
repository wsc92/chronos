...
#include "../../../engine/src/memory/dynamic_allocator.h"
...
u8 dynamic_allocator_should_create_and_destroy() {
    ...
    void* memory = callocate(memory_requirement, MEMORY_TAG_APPLICATION);
    ...
    cfree(memory, memory_requirement, MEMORY_TAG_APPLICATION);
    return true;
}
...
u8 dynamic_allocator_multi_allocation_over_allocate() {
    ...
    CDEBUG("Note: The following warning and errors are intentionally caused by this test.");
...
u8 util_free(dynamic_allocator* allocator, alloc_data* data, u64* currently_allocated, u64 header_size, u64 total_allocator_size) {
    if (!dynamic_allocator_free_aligned(allocator, data->block)) {
        CERROR("util_free, dynamic_allocator_free_aligned failed");
        return false;
    }
    data->block = 0;
    currently_allocated -= (data->size + header_size + data->alignment);
...
u8 dynamic_allocator_multiple_alloc_aligned_different_alignments_random() {
    ...
    for (u32 i = 0; i < alloc_data_count; ++i) {
        alloc_datas[i].alignment = po2[crandom_in_range(0, 7)];
        alloc_datas[i].size = (u64)crandom_in_range(1, 65536);
    }
    ...
    while (alloc_count != alloc_data_count) {
        u32 index = (u32)crandom_in_range(0, alloc_data_count);
        if (alloc_datas[index].block == 0) {
            if (!util_allocate(&alloc, &alloc_datas[index], &currently_allocated, header_size, total_allocator_size)) {
                CERROR("util_allocate failed on index: %u.");
                return false;
            }
            alloc_count++;
        }
    ...
    CTRACE("Randomly allocated %u times. Freeing randomly...", alloc_count);
    ...
    while (alloc_count != alloc_data_count) {
        ...
        if (alloc_datas[index].block != 0) {
            if (!util_free(&alloc, &alloc_datas[index], &currently_allocated, header_size, total_allocator_size)) {
                CERROR("util_free failed on index: %u.");
                return false;
            }
            alloc_count--;
        }
...
u8 dynamic_allocator_multiple_alloc_and_free_aligned_different_alignments_random() {
    ...
    while (op_count < max_op_count) {
        ...
        if (alloc_count == 0 || crandom_in_range(0, 99) > 50) {
            while (true) {
                u32 index = (u32)crandom_in_range(0, alloc_data_count - 1);
                if (alloc_datas[index].block == 0) {
                    if (!util_allocate(&alloc, &alloc_datas[index], &currently_allocated, header_size, total_allocator_size)) {
                        CERROR("util_allocate failed on index: %u.", index);
                        return false;
                    }
                    alloc_count++;
                    break;
            ...
            while (true) {
                ...
                if (alloc_datas[index].block != 0) {
                    if (!util_free(&alloc, &alloc_datas[index], &currently_allocated, header_size, total_allocator_size)) {
                        CERROR("util_free failed on index: %u.", index);
                        return false;
                    }
                    alloc_count--;
                    break;
    ...
    CTRACE("Max op count of %u reached. Freeing remaining allocations.", max_op_count);
    for (u32 i = 0; i < alloc_data_count; ++i) {
