...
i32 main(i32 argc, char** argv) {
    ...
    if (argc < 2) {
        CERROR("chronos tools requires at least one argument.");
        print_help();
        return -1;
    }
    ...
    if (strings_equali(argv[1], "buildshaders") || strings_equali(argv[1], "bshaders")) {
        ...
        CERROR("Unrecognized argument '%s'.", argv[1]);
        print_help();
        return -2;
    }
...
i32 process_shaders(i32 argc, char** argv) {
    if (argc < 3) {
        CERROR("Build shaders mode requires at least one additional argument.");
        return -3;
    }
    ...
    for (u32 i = 2; i < argc; ++i) {
        ...
        // Some output.
        CINFO("Processing %s -> %s...", argv[i], out_filename);
        ...
        if (retcode != 0) {
            CERROR("Error compiling shader. See logs. Aborting process.");
            return -5;
        }
    }

    CINFO("Successfully processed all shaders.");
    return 0;
}
