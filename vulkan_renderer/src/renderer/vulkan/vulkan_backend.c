...
// #define CVULKAN_ALLOCATOR_TRACE 1
// #endif

// NOTE: To disable the custom allocator, comment this out or set to 0.
#ifndef CVULKAN_USE_CUSTOM_ALLOCATOR
#define CVULKAN_USE_CUSTOM_ALLOCATOR 1
#endif
...
void vulkan_alloc_free(void* user_data, void* memory) {
    if (!memory) {
#ifdef CVULKAN_ALLOCATOR_TRACE
        CTRACE("Block is null, nothing to free: %p", memory);
#endif
        return;
    }

#ifdef CVULKAN_ALLOCATOR_TRACE
    CTRACE("Attempting to free block %p...", memory);
#endif
    u64 size;
    u16 alignment;
    b8 result = cmemory_get_size_alignment(memory, &size, &alignment);
    if (result) {
#ifdef CVULKAN_ALLOCATOR_TRACE
        CTRACE("Block %p found with size/alignment: %llu/%u. Freeing aligned block...", memory, size, alignment);
#endif
        cfree_aligned(memory, size, alignment, MEMORY_TAG_VULKAN);
    } else {
        CERROR("vulkan_alloc_free failed to get alignment lookup for block %p.", memory);
    }
}
...
void* vulkan_alloc_reallocation(void* user_data, void* original, size_t size, size_t alignment, VkSystemAllocationScope allocation_scope) {
    ...
    u64 alloc_size;
    u16 alloc_alignment;
    b8 is_aligned = cmemory_get_size_alignment(original, &alloc_size, &alloc_alignment);
    if (!is_aligned) {
        CERROR("vulkan_alloc_reallocation of unaligned block %p", original);
        return 0;
    }

    if (alloc_alignment != alignment) {
        CERROR("Attempted realloc using a different alignment of %llu than the original of %hu.", alignment, alloc_alignment);
        return 0;
    }

#ifdef CVULKAN_ALLOCATOR_TRACE
    CTRACE("Attempting to realloc block %p...", original);
#endif

    void* result = vulkan_alloc_allocation(user_data, size, alloc_alignment, allocation_scope);
    if (result) {
#ifdef CVULKAN_ALLOCATOR_TRACE
        CTRACE("Block %p reallocated to %p, copying data...", original, result);
#endif

        // Copy over the original memory.
        ccopy_memory(result, original, size);
#ifdef CVULKAN_ALLOCATOR_TRACE
        CTRACE("Freeing original aligned block %p...", original);
#endif
        // Free the original memory only if the new allocation was successful.
        cfree_aligned(original, alloc_size, alloc_alignment, MEMORY_TAG_VULKAN);
    } else {
#ifdef CVULKAN_ALLOCATOR_TRACE
        CERROR("Failed to realloc %p.", original);
#endif
    }
...
#ifdef CVULKAN_ALLOCATOR_TRACE
    CTRACE("External allocation of size: %llu", size);
#endif
    callocate_report((u64)size, MEMORY_TAG_VULKAN_EXT);
}
...
#ifdef CVULKAN_ALLOCATOR_TRACE
    CTRACE("External free of size: %llu", size);
#endif
    cfree_report((u64)size, MEMORY_TAG_VULKAN_EXT);
}
...
#if CVULKAN_USE_CUSTOM_ALLOCATOR == 1
    context.allocator = callocate(sizeof(VkAllocationCallbacks), MEMORY_TAG_RENDERER);
    if (!create_vulkan_allocator(context.allocator)) {
        // If this fails, gracefully fall back to the default allocator.
        CFATAL("Failed to create custom Vulkan allocator. Continuing using the driver's default allocator.");
        cfree(context.allocator, sizeof(VkAllocationCallbacks), MEMORY_TAG_RENDERER);
        context.allocator = 0;
    }
#else
    context.allocator = 0;
    ...
    for (u32 i = 0; i < required_extension_count; ++i) {
        ...
        for (u32 j = 0; j < available_extension_count; ++j) {
            if (strings_equal(required_extensions[i], available_extensions[j].extensionName)) {
                found = true;
                CINFO("Required exension found: %s...", required_extensions[i]);
                break;
            }
        }

        if (!found) {
            CFATAL("Required extension is missing: %s", required_extensions[i]);
            return false;
        }
    }
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    for (u32 i = 0; i < required_validation_layer_count; ++i) {
        ...
        for (u32 j = 0; j < available_layer_count; ++j) {
            if (strings_equal(required_validation_layer_names[i], available_layers[j].layerName)) {
                found = true;
                CINFO("Found validation layer: %s...", required_validation_layer_names[i]);
                break;
            }
        }
    ...
    if (!vulkan_result_is_success(instance_result)) {
        ...
        CFATAL("Vulkan instance creation failed with result: '%s'", result_string);
        return false;
    }
    ...
    *out_window_render_target_count = context.swapchain.image_count;
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_INDEX, index_buffer_size, true, &context.object_index_buffer)) {
        CERROR("Error creating index buffer.");
        return false;
    }
...
b8 vulkan_renderer_backend_begin_frame(renderer_plugin* backend, f32 delta_time) {
    ...
    if (!vulkan_result_is_success(result)) {
        CFATAL("In-flight fence wait failure! error: %s", vulkan_result_string(result, true));
        return false;
    }
    ...
    if (!vulkan_swapchain_acquire_next_image_index(
            ...
            0,
            &context.image_index)) {
        CERROR("Failed to acquire next image index, booting.");
        return false;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_plugin* backend, f32 delta_time) {
    ...
    if (context.images_in_flight[context.image_index] != VK_NULL_HANDLE) {  // was frame
        ...
        if (!vulkan_result_is_success(result)) {
            CFATAL("vkWaitForFences error: %s", vulkan_result_string(result, true));
        }
    }
...
b8 vulkan_renderer_renderpass_begin(renderpass* pass, render_target* target) {
    ...
    if (do_clear_colour) {
        ccopy_memory(clear_values[begin_info.clearValueCount].color.float32, pass->clear_colour.elements, sizeof(f32) * 4);
...
void vulkan_renderer_texture_create(const u8* pixels, texture* t) {
    ...
    t->internal_data = (vulkan_image*)callocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
...
void vulkan_renderer_texture_destroy(struct texture* texture) {
    ...
    if (image) {
        ...
        czero_memory(image, sizeof(vulkan_image));

        cfree(texture->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
...
void vulkan_renderer_texture_write_data(texture* t, u32 offset, u32 size, const u8* pixels) {
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_STAGING, size, false, &staging)) {
        CERROR("Failed to create staging buffer for texture write.");
        return;
    }
...
void vulkan_renderer_texture_read_data(texture* t, u32 offset, u32 size, void** out_memory) {
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, size, false, &staging)) {
        CERROR("Failed to create staging buffer for texture read.");
        return;
    }
    renderer_renderbuffer_bind(&staging, 0);
    ...
    if (!vulkan_buffer_read(&staging, offset, size, out_memory)) {
        CERROR("vulkan_buffer_read failed.");
    }
...
void vulkan_renderer_texture_read_pixel(texture* t, u32 x, u32 y, u8** out_rgba) {
    ...
    if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, sizeof(u8) * 4, false, &staging)) {
        CERROR("Failed to create staging buffer for texture pixel read.");
        return;
    }
    renderer_renderbuffer_bind(&staging, 0);
...
b8 vulkan_renderer_create_geometry(geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
    if (!vertex_count || !vertices) {
        CERROR("vulkan_renderer_create_geometry requires vertex data, and none was supplied. vertex_count=%d, vertices=%p", vertex_count, vertices);
        return false;
    }
    ...
    if (!internal_data) {
        CFATAL("vulkan_renderer_create_geometry failed to find a free index for a new geometry upload. Adjust config to allow for more.");
        return false;
    }
    ...
    if (!renderer_renderbuffer_allocate(&context.object_vertex_buffer, total_size, &internal_data->vertex_buffer_offset)) {
        CERROR("vulkan_renderer_create_geometry failed to allocate from the vertex buffer!");
        return false;
    }

    // Load the data.
    if (!renderer_renderbuffer_load_range(&context.object_vertex_buffer, internal_data->vertex_buffer_offset, total_size, vertices)) {
        CERROR("vulkan_renderer_create_geometry failed to upload to the vertex buffer!");
        return false;
    }
    ...
    if (index_count && indices) {
        ...
        if (!renderer_renderbuffer_allocate(&context.object_index_buffer, total_size, &internal_data->index_buffer_offset)) {
            CERROR("vulkan_renderer_create_geometry failed to allocate from the index buffer!");
            return false;
        }

        if (!renderer_renderbuffer_load_range(&context.object_index_buffer, internal_data->index_buffer_offset, total_size, indices)) {
            CERROR("vulkan_renderer_create_geometry failed to upload to the index buffer!");
            return false;
        }
    }
    ...
    if (is_reupload) {
        ...
        if (!renderer_renderbuffer_free(&context.object_vertex_buffer, old_range.vertex_element_size * old_range.vertex_count, old_range.vertex_buffer_offset)) {
            CERROR("vulkan_renderer_create_geometry free operation failed during reupload of vertex data.");
            return false;
        }
        ...
        if (old_range.index_element_size > 0) {
            if (!renderer_renderbuffer_free(&context.object_index_buffer, old_range.index_element_size * old_range.index_count, old_range.index_buffer_offset)) {
                CERROR("vulkan_renderer_create_geometry free operation failed during reupload of index data.");
                return false;
            }
        }
    }
...
void vulkan_renderer_destroy_geometry(geometry* geometry) {
    if (geometry && geometry->internal_id != INVALID_ID) {
        ...
        if (!renderer_renderbuffer_free(&context.object_vertex_buffer, internal_data->vertex_element_size * internal_data->vertex_count, internal_data->vertex_buffer_offset)) {
            CERROR("vulkan_renderer_destroy_geometry failed to free vertex buffer range.");
        }
        ...
        if (internal_data->index_element_size > 0) {
            if (!renderer_renderbuffer_free(&context.object_index_buffer, internal_data->index_element_size * internal_data->index_count, internal_data->index_buffer_offset)) {
                CERROR("vulkan_renderer_destroy_geometry failed to free index buffer range.");
            }
        }
...
void vulkan_renderer_draw_geometry(geometry_render_data* data) {
    ...
    if (!vulkan_buffer_draw(&context.object_vertex_buffer, buffer_data->vertex_buffer_offset, buffer_data->vertex_count, includes_index_data)) {
        CERROR("vulkan_renderer_draw_geometry failed to draw vertex buffer;");
        return;
    }
    ...
    if (includes_index_data) {
        if (!vulkan_buffer_draw(&context.object_index_buffer, buffer_data->index_buffer_offset, buffer_data->index_count, !includes_index_data)) {
            CERROR("vulkan_renderer_draw_geometry failed to draw index buffer;");
            return;
        }
    }
}
...
b8 vulkan_renderer_shader_create(shader* s, const shader_config* config, renderpass* pass, u8 stage_count, const char** stage_filenames, shader_stage* stages) {
    s->internal_data = callocate(sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
    ...
    for (u8 i = 0; i < stage_count; ++i) {
        switch (stages[i]) {
            ...
            case SHADER_STAGE_GEOMETRY:
                CWARN("vulkan_renderer_shader_create: VK_SHADER_STAGE_GEOMETRY_BIT is set but not yet supported.");
                vk_stages[i] = VK_SHADER_STAGE_GEOMETRY_BIT;
                break;
            case SHADER_STAGE_COMPUTE:
                CWARN("vulkan_renderer_shader_create: SHADER_STAGE_COMPUTE is set but not yet supported.");
                vk_stages[i] = VK_SHADER_STAGE_COMPUTE_BIT;
                break;
            default:
                CERROR("Unsupported stage type: %d", stages[i]);
                break;
        }
    }
    ...
    czero_memory(internal_shader->config.stages, sizeof(vulkan_shader_stage_config) * VULKAN_SHADER_MAX_STAGES);
    ...
    for (u32 i = 0; i < stage_count; i++) {
        ...
        if (internal_shader->config.stage_count + 1 > VULKAN_SHADER_MAX_STAGES) {
            CERROR("Shaders may have a maximum of %d stages", VULKAN_SHADER_MAX_STAGES);
            return false;
        }
        ...
        switch (stages[i]) {
            ...
            default:
                // Go to the next type.
                CERROR("vulkan_shader_create: Unsupported shader stage flagged: %d. Stage ignored.", stages[i]);
                continue;
        }
    ...
    czero_memory(internal_shader->config.descriptor_sets, sizeof(vulkan_descriptor_set_config) * 2);
    ...
    // Attributes array.
    czero_memory(internal_shader->config.attributes, sizeof(VkVertexInputAttributeDescription) * VULKAN_SHADER_MAX_ATTRIBUTES);
...
void vulkan_renderer_shader_destroy(shader* s) {
    if (s && s->internal_data) {
        ...
        if (!shader) {
            CERROR("vulkan_renderer_shader_destroy requires a valid pointer to a shader.");
            return;
        }
        ...
        czero_memory(&shader->config, sizeof(vulkan_shader_config));

        // Free the internal data memory.
        cfree(s->internal_data, sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
        s->internal_data = 0;
    }
}
...
b8 vulkan_renderer_shader_initialize(shader* s) {
    ...
    czero_memory(internal_shader->stages, sizeof(vulkan_shader_stage) * VULKAN_SHADER_MAX_STAGES);
    for (u32 i = 0; i < internal_shader->config.stage_count; ++i) {
        if (!create_module(internal_shader, internal_shader->config.stages[i], &internal_shader->stages[i])) {
            CERROR("Unable to create %s shader module for '%s'. Shader will be destroyed.", internal_shader->config.stages[i].file_name, s->name);
            return false;
        }
    }
    ...
    if (!vulkan_result_is_success(result)) {
        CERROR("vulkan_shader_initialize failed creating descriptor pool: '%s'", vulkan_result_string(result, true));
        return false;
    }

    // Create descriptor set layouts.
    czero_memory(internal_shader->descriptor_set_layouts, internal_shader->config.descriptor_set_count);
    ...
    czero_memory(stage_create_infos, sizeof(VkPipelineShaderStageCreateInfo) * VULKAN_SHADER_MAX_STAGES);
    for (u32 i = 0; i < internal_shader->config.stage_count; ++i) {
    ...
    if (!renderer_renderbuffer_allocate(&internal_shader->uniform_buffer, s->global_ubo_stride, &s->global_ubo_offset)) {
        CERROR("Failed to allocate space for the uniform buffer!");
        return false;
    }
...
b8 vulkan_renderer_shader_bind_instance(shader* s, u32 instance_id) {
    if (!s) {
        CERROR("vulkan_shader_bind_instance requires a valid pointer to a shader.");
        return false;
    }
...
b8 vulkan_renderer_shader_apply_globals(shader* s) {
    ...
    if (global_set_binding_count > 1) {
        ...
        CERROR("Global image samplers are not yet supported.");
...
b8 vulkan_renderer_shader_apply_instance(shader* s, b8 needs_update) {
    ...
    if (internal->instance_uniform_count < 1 && internal->instance_uniform_sampler_count < 1) {
        CERROR("This shader does not use instances.");
        return false;
    }
    u32 image_index = context.image_index;
    VkCommandBuffer command_buffer = context.graphics_command_buffers[image_index].handle;
    ...
    if (needs_update) {
        ...
        czero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * 2);
                ...
                if (t->generation == INVALID_ID) {
                    switch (map->use) {
                        ...
                        case TEXTURE_USE_MAP_NORMAL:
                            ...
                            break;
                        default:
                            CWARN("Undefined texture use %d", map->use);
...
VkSamplerAddressMode convert_repeat_type(const char* axis, texture_repeat repeat) {
    switch (repeat) {
        ...
        default:
            CWARN("convert_repeat_type(axis='%s') Type '%x' not supported, defaulting to repeat.", axis, repeat);
            return VK_SAMPLER_ADDRESS_MODE_REPEAT;
    }
}
...
VkFilter convert_filter_type(const char* op, texture_filter filter) {
    switch (filter) {
        ...
        default:
            CWARN("convert_filter_type(op='%s'): Unsupported filter type '%x', defaulting to linear.", op, filter);
            return VK_FILTER_LINEAR;
    }
}
...
b8 vulkan_renderer_texture_map_acquire_resources(texture_map* map) {
    ...
    if (!vulkan_result_is_success(VK_SUCCESS)) {
        CERROR("Error creating texture sampler: %s", vulkan_result_string(result, true));
        return false;
    }
...
b8 vulkan_renderer_shader_acquire_instance_resources(shader* s, texture_map** maps, u32* out_instance_id) {
    ...
    *out_instance_id = INVALID_ID;
    ...
    if (*out_instance_id == INVALID_ID) {
        CERROR("vulkan_shader_acquire_instance_resources failed to acquire new id");
        return false;
    }
    ...
    if (s->instance_texture_count > 0) {
        ...
        instance_state->instance_texture_maps = callocate(sizeof(texture_map*) * s->instance_texture_count, MEMORY_TAG_ARRAY);
        texture* default_texture = texture_system_get_default_texture();
        ccopy_memory(instance_state->instance_texture_maps, maps, sizeof(texture_map*) * s->instance_texture_count);
    ...
    if (size > 0) {
        if (!renderer_renderbuffer_allocate(&internal->uniform_buffer, size, &instance_state->offset)) {
            CERROR("vulkan_material_shader_acquire_resources failed to acquire ubo space");
            return false;
        }
    }
    ...
    czero_memory(set_state->descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);
    ...
    if (result != VK_SUCCESS) {
        CERROR("Error allocating instance descriptor sets in shader: '%s'.", vulkan_result_string(result, true));
        return false;
    }
...
b8 vulkan_renderer_shader_release_instance_resources(shader* s, u32 instance_id) {
    ...
    VkResult result = vkFreeDescriptorSets(
        ...
        instance_state->descriptor_set_state.descriptor_sets);
    if (result != VK_SUCCESS) {
        CERROR("Error freeing object shader descriptor sets!");
    }

    // Destroy descriptor states.
    czero_memory(instance_state->descriptor_set_state.descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);

    if (instance_state->instance_texture_maps) {
        cfree(instance_state->instance_texture_maps, sizeof(texture_map*) * s->instance_texture_count, MEMORY_TAG_ARRAY);
    ...
    if (!renderer_renderbuffer_free(&internal->uniform_buffer, s->ubo_stride, instance_state->offset)) {
        CERROR("vulkan_renderer_shader_release_instance_resources failed to free range from renderbuffer.");
    }
...
b8 vulkan_renderer_set_uniform(shader* s, shader_uniform* uniform, const void* value) {
    ...
    if (uniform->type == SHADER_UNIFORM_TYPE_SAMPLER) {
        ...
        if (uniform->scope == SHADER_SCOPE_LOCAL) {
            ...
            ccopy_memory((void*)addr, value, uniform->size);
            if (addr) {
            }
        }
    }
...
b8 create_module(vulkan_shader* shader, vulkan_shader_stage_config config, vulkan_shader_stage* shader_stage) {
    ...
    if (!resource_system_load(config.file_name, RESOURCE_TYPE_BINARY, 0, &binary_resource)) {
        CERROR("Unable to read shader module: %s.", config.file_name);
        return false;
    }

    czero_memory(&shader_stage->create_info, sizeof(VkShaderModuleCreateInfo));
    ...
    // Shader stage info
    czero_memory(&shader_stage->shader_stage_create_info, sizeof(VkPipelineShaderStageCreateInfo));
...
b8 vulkan_renderpass_create(const renderpass_config* config, renderpass* out_renderpass) {
    out_renderpass->internal_data = callocate(sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
        ...
        if (attachment_config->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
            ...
            if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_DONT_CARE) {
                ...
                if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD) {
                    if (do_clear_colour) {
                        CWARN("Colour attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration.");
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                    } else {
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
                    }
                } else {
                    CFATAL("Invalid and unsupported combination of load operation (0x%x) and clear flags (0x%x) for colour attachment.", attachment_desc.loadOp, out_renderpass->clear_flags);
                    return false;
                }
            }
            ...
            if (attachment_config->store_operation == RENDER_TARGET_ATTACHMENT_STORE_OPERATION_DONT_CARE) {
                ...
                CFATAL("Invalid store operation (0x%x) set for depth attachment. Check configuration.", attachment_config->store_operation);
                return false;
            }
            ...
            if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_DONT_CARE) {
                ...
                if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD) {
                    if (do_clear_depth) {
                        CWARN("Depth attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration.");
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                    } else {
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
                    }
                } else {
                    CFATAL("Invalid and unsupported combination of load operation (0x%x) and clear flags (0x%x) for depth attachment.", attachment_desc.loadOp, out_renderpass->clear_flags);
                    return false;
                }
            }
    ...
    if (colour_attachment_count > 0) {
        colour_attachment_references = callocate(sizeof(VkAttachmentReference) * colour_attachment_count, MEMORY_TAG_ARRAY);
    ...
    if (depth_attachment_count > 0) {
        CASSERT_MSG(depth_attachment_count == 1, "Multiple depth attachments not supported.");
        depth_attachment_references = callocate(sizeof(VkAttachmentReference) * depth_attachment_count, MEMORY_TAG_ARRAY);
    ...
    if (colour_attachment_references) {
        cfree(colour_attachment_references, sizeof(VkAttachmentReference) * colour_attachment_count, MEMORY_TAG_ARRAY);
    }
    ...
    if (depth_attachment_references) {
        cfree(depth_attachment_references, sizeof(VkAttachmentReference) * depth_attachment_count, MEMORY_TAG_ARRAY);
    }
...
void vulkan_renderpass_destroy(renderpass* pass) {
    if (pass && pass->internal_data) {
        ...
        cfree(internal_data, sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
...
b8 vulkan_renderer_render_target_create(u8 attachment_count, render_target_attachment* attachments, renderpass* pass, u32 width, u32 height, render_target* out_target) {
    ...
    ccopy_memory(out_target->attachments, attachments, sizeof(render_target_attachment) * attachment_count);
...
void vulkan_renderer_render_target_destroy(render_target* target, b8 free_internal_memory) {
    if (target && target->internal_framebuffer) {
        ...
        if (free_internal_memory) {
            cfree(target->attachments, sizeof(render_target_attachment) * target->attachment_count, MEMORY_TAG_ARRAY);
...
texture* vulkan_renderer_window_attachment_get(u8 index) {
    if (index >= context.swapchain.image_count) {
        CFATAL("Attempting to get colour attachment index out of range: %d. Attachment count: %d", index, context.swapchain.image_count);
        return 0;
    }
...
texture* vulkan_renderer_depth_attachment_get(u8 index) {
    if (index >= context.swapchain.image_count) {
        CFATAL("Attempting to get depth attachment index out of range: %d. Attachment count: %d", index, context.swapchain.image_count);
        return 0;
    }
...
b8 vulkan_buffer_create_internal(renderbuffer* buffer) {
    if (!buffer) {
        CERROR("vulkan_buffer_create_internal requires a valid pointer to a buffer.");
        return false;
    }
    ...
    switch (buffer->type) {
        ...
        case RENDERBUFFER_TYPE_STORAGE:
            CERROR("Storage buffer not yet supported.");
            return false;
        default:
            CERROR("Unsupported buffer type: %i", buffer->type);
            return false;
    }
    ...
    callocate_report(internal_buffer.memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
    ...
    buffer->internal_data = callocate(sizeof(vulkan_buffer), MEMORY_TAG_VULKAN);
...
void vulkan_buffer_destroy_internal(renderbuffer* buffer) {
    if (buffer) {
        ...
        if (internal_buffer) {
            ...
            b8 is_device_memory = (internal_buffer->memory_property_flags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
            cfree_report(internal_buffer->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
            czero_memory(&internal_buffer->memory_requirements, sizeof(VkMemoryRequirements));

            internal_buffer->usage = 0;
            internal_buffer->is_locked = false;

            // Free up the internal buffer.
            cfree(buffer->internal_data, sizeof(vulkan_buffer), MEMORY_TAG_VULKAN);
...
b8 vulkan_buffer_bind(renderbuffer* buffer, u64 offset) {
    if (!buffer || !buffer->internal_data) {
        CERROR("vulkan_buffer_bind requires valid pointer to a buffer.");
        return false;
    }
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
...
b8 vulkan_buffer_unbind(renderbuffer* buffer) {
    if (!buffer || !buffer->internal_data) {
        CERROR("vulkan_buffer_unbind requires valid pointer to a buffer.");
        return false;
    }
...
void* vulkan_buffer_map_memory(renderbuffer* buffer, u64 offset, u64 size) {
    if (!buffer || !buffer->internal_data) {
        CERROR("vulkan_buffer_map_memory requires a valid pointer to a buffer.");
        return 0;
    }
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
    void* data;
...
void vulkan_buffer_unmap_memory(renderbuffer* buffer, u64 offset, u64 size) {
    if (!buffer || !buffer->internal_data) {
        CERROR("vulkan_buffer_unmap_memory requires a valid pointer to a buffer.");
        return;
    }
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
...
b8 vulkan_buffer_flush(renderbuffer* buffer, u64 offset, u64 size) {
    if (!buffer || !buffer->internal_data) {
        CERROR("vulkan_buffer_flush requires a valid pointer to a buffer.");
        return false;
    }
...
b8 vulkan_buffer_read(renderbuffer* buffer, u64 offset, u64 size, void** out_memory) {
    if (!buffer || !buffer->internal_data || !out_memory) {
        CERROR("vulkan_buffer_read requires a valid pointer to a buffer and out_memory, and the size must be nonzero.");
        return false;
    }
    ...
    if (vulkan_buffer_is_device_local(internal_buffer) && !vulkan_buffer_is_host_visible(internal_buffer)) {
        ...
        if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, size, false, &read)) {
            CERROR("vulkan_buffer_read() - Failed to create read buffer.");
            return false;
        }
        ...
        ccopy_memory(*out_memory, mapped_data, size);
        ...
        ccopy_memory(*out_memory, data_ptr, size);
        vkUnmapMemory(context.device.logical_device, internal_buffer->memory);
    }
...
b8 vulkan_buffer_load_range(renderbuffer* buffer, u64 offset, u64 size, const void* data) {
    if (!buffer || !buffer->internal_data || !size || !data) {
        CERROR("vulkan_buffer_load_range requires a valid pointer to a buffer, a nonzero size and a valid pointer to data.");
        return false;
    }
    ...
    if (vulkan_buffer_is_device_local(internal_buffer) && !vulkan_buffer_is_host_visible(internal_buffer)) {
        ...
        renderbuffer staging;
        if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_STAGING, size, false, &staging)) {
            CERROR("vulkan_buffer_load_range() - Failed to create staging buffer.");
            return false;
        }
        renderer_renderbuffer_bind(&staging, 0);
...
b8 vulkan_buffer_copy_range(renderbuffer* source, u64 source_offset, renderbuffer* dest, u64 dest_offset, u64 size) {
    if (!source || !source->internal_data || !dest || !dest->internal_data || !size) {
        CERROR("vulkan_buffer_copy_range requires a valid pointers to source and destination buffers as well as a nonzero size.");
        return false;
    }
...
b8 vulkan_buffer_draw(renderbuffer* buffer, u64 offset, u32 element_count, b8 bind_only) {
    ...
    if (buffer->type == RENDERBUFFER_TYPE_VERTEX) {
        ...
        CERROR("Cannot draw buffer of type: %i", buffer->type);
        return false;
    }
}
